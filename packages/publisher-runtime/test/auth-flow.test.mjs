...
async function fixture(t, scopes = ['creator.profile.read', 'creator.studio-draft.write']) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    res.end(JSON.stringify({ token: 'private-test-token', expiresIn: 3600, scopes, accountHint: 'te***@example.test' }));
  });
  ...
  return { root, run, start, exchanges, env };
}
