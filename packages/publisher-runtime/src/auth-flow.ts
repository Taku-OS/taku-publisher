...
export type AuthFlowOptions = Pick<BrowserAuthOptions, 'workerUrl' | 'siteUrl' | 'intent' | 'timeoutMs' | 'openBrowser'> & { requiredScopes: string[] };
...
export async function checkAuthFlow(requestId?: string, env = process.env): Promise<JsonObject> {
  ...
  if (status === 'authenticated') {
    ...
    if (session?.createdAt !== state.session_created_at || auth.source !== 'publisher_session' || !auth.token || !scopes.every(scope => authHasScope(auth, scope))) {
...
export async function startAuthFlow(options: AuthFlowOptions, env = process.env): Promise<JsonObject> {
  return withLock(env, async () => {
    ...
    if (previous && activeAuthStates.has(String(previous.status)) && processAlive(Number(previous.pid))) {
      if (previous.intent !== (options.intent ?? 'publish_tool') || previous.worker_url !== options.workerUrl || previous.site_url !== (options.siteUrl ?? 'https://taku.ai') || JSON.stringify(previous.required_scopes) !== JSON.stringify(options.requiredScopes)) {
