...
const CREATOR_COMMANDS = new Map([
  ...
  ['creator-challenge-select', 'challenge-select'],
  ['creator-challenge-status', 'challenge-status'],
  ['creator-challenge-skip', 'challenge-skip'],
  ['creator-challenge-prepare', 'challenge-prepare'],
...
const STAX_CHALLENGE_SELECTION_GUIDANCE = [
  'Choose one candidate Skill to prepare it for review and publishing, or skip.',
  'Publishing an eligible Skill or creating a Taku App can help complete the Stax Challenge and qualify for campaign rewards; see the Challenge page for eligibility and reward terms.',
  'Choosing a candidate does not publish it; final public release still requires your confirmation.',
].join(' ');
...
async function dispatchCommand(args: ParsedArguments): Promise<JsonObject> {
  ...
  if (args.command === 'marketplace-search') {
    ...
    if (limit < 1 || limit > 100 || offset < 0) throw new PublisherError('Marketplace limit must be 1-100 and offset cannot be negative.', 'invalid_marketplace_pagination');
    const kind = (optionalFlag(args, 'kind') ?? optionalFlag(args, 'type') ?? 'all').toLowerCase();
    if (!MARKETPLACE_KINDS.has(kind)) throw new PublisherError('Marketplace kind is not supported.', 'invalid_marketplace_kind', { kind });
    const client = publicMarketplaceClient(args);
    const response = await client.searchMarketplace(
      stringFlag(args, 'search', ''),
      kind,
      limit,
      offset,
    );
  ...
  if (args.command === 'subapp-assess') {
    ...
    const assessed = await assessSubAppSource(
      ...
      },
      {
        ...
        serviceCatalogTimeoutMs:
          numberFlag(args, 'service-catalog-timeout', 15) * 1000,
        serviceSearchTimeoutMs:
          numberFlag(args, 'service-search-timeout', 15) * 1000,
        serviceSearchLimit: numberFlag(args, 'service-search-limit', 5),
        ...(serviceMappingsFile
          ? {
              serviceMappings: await readObject(
                serviceMappingsFile,
                'SubApp service mappings must be a JSON object.',
                'subapp_service_mappings_invalid',
              ),
            }
          : {}),
      },
    ...
    const assessmentReview = assessmentReviewFile
      ? validateSubAppAssessmentReview(
          ...
          await readObject(
            ...
            'SubApp assessment review must be a JSON object.',
            'subapp_assessment_review_invalid',
          ),
        )
      : undefined;
    const conversionCanStart =
      assessment.eligibility === 'eligible' || Boolean(assessmentReview);
    const hasUnresolvedServiceMapping = assessment.serviceRequirements.some(
      (requirement) =>
    ...
    const hasServiceSuggestions =
      assessed.serviceDiscovery?.requirements.some(
        (requirement) => requirement.candidates.length > 0,
      ) === true;
    ...
    return jsonOutput(status, {
      ...
      service_discovery:
        (assessed.serviceDiscovery as unknown as JsonValue) ?? null,
      service_mapping_review:
        (assessed.serviceMappingReview as unknown as JsonValue) ?? null,
      assessment_review_template:
        (reviewTemplate as unknown as JsonValue) ?? null,
      assessment_review:
        (assessmentReview as unknown as JsonValue) ?? null,
  ...
  if (args.command === 'subapp-prepare') {
    ...
    const prepared = await prepareSubAppCandidate(
      ...
      },
      {
        ...
        ...(assessmentReviewFile
          ? {
              assessmentReview: await readObject(
                assessmentReviewFile,
                'SubApp assessment review must be a JSON object.',
                'subapp_assessment_review_invalid',
              ),
    ...
    return jsonOutput('subapp_candidate_prepared', {
      ...
      assessment: prepared.assessment as unknown as JsonValue,
      converter: prepared.converter,
      service_catalog: prepared.serviceCatalog ?? null,
      service_discovery:
        (prepared.serviceDiscovery as unknown as JsonValue) ?? null,
      service_mapping_review:
        (prepared.serviceMappingReview as unknown as JsonValue) ?? null,
      assessment_review:
        (prepared.assessmentReview as unknown as JsonValue) ?? null,
  ...
  if (args.command === 'subapp-convert') {
    const handoff = await createSubAppAgentHandoff(
      requiredFlag(args, 'candidate'),
      {
        converterBin: optionalFlag(args, 'converter-bin'),
        timeoutMs: numberFlag(args, 'timeout', 120) * 1000,
  ...
  if (args.command === 'subapp-conversion-check') {
    const checked = await checkSubAppConversion(
      requiredFlag(args, 'candidate'),
      {
        converterBin: optionalFlag(args, 'converter-bin'),
        timeoutMs: numberFlag(args, 'timeout', 120) * 1000,
    ...
    return jsonOutput(
        ...
        : 'subapp_conversion_needs_work',
      {
        ...
        validation: checked.validation as unknown as JsonValue,
  ...
  if (args.command === 'subapp-runtime-plan') {
    const planned = await planSubAppRuntimeValidation(
      requiredFlag(args, 'candidate'),
      {
        converterBin: optionalFlag(args, 'converter-bin'),
        timeoutMs: numberFlag(args, 'timeout', 120) * 1000,
  ...
  if (args.command === 'subapp-runtime-check') {
    ...
    return jsonOutput(
        ...
        : 'subapp_trusted_runtime_failed',
      {
        ...
        scripts_executed_in_disposable_workspace:
          result.scriptsExecutedInDisposableWorkspace,
        publish_started: false,
  ...
  if (args.command === 'subapp-register-plan') {
    const planned = await planSubAppRegistration({
      packageRoot: requiredFlag(args, 'package-root'),
      metadata: await readObject(
        requiredFlag(args, 'metadata'),
        'SubApp registration metadata must be a JSON object.',
  ...
  if (args.command === 'subapp-register') {
    ...
    const registered = await registerSubAppDraft(
      registrationRequest,
      client,
    );
  ...
  if (args.command === 'stage') return jsonOutput('staged', { draft_id: draftId, ...await stageSelected(directory, state) });
  ...
  const client = await authenticatedClient(args);
  if (args.command === 'remote-create') return remoteCreate(args, client, directory, state);
  const remoteId = requireRemoteId(state);
  if (args.command === 'remote-get') return jsonOutput('remote_draft', { remote_draft: await client.getDraft(remoteId) });
...
async function remoteCreate(
  ...
  if (state.mode !== 'update' && !String(listing.iconUrl ?? '').trim()) {
    try {
      ...
      if (!iconUrl || !publicHttpsUrl(iconUrl)) throw new PublisherError('Taku icon generation did not return a valid HTTPS image URL.', 'invalid_generated_icon');
...
async function runCreatorCommand(command: string, creatorArgs: string[]): Promise<JsonObject> {
  ...
  if (cloudStudio && payload.ok === true) {
    ...
    if (payload.challengeHandoff === true) {
      const savedMessage = accountHint
        ? `The private Card was saved to Taku account ${accountHint}.`
        : 'The private Card was saved to the current Taku account.';
      payload.message = `${savedMessage} Open the Stax Challenge Review editorUrl. ${STAX_CHALLENGE_SELECTION_GUIDANCE}`;
    } else {
...
function creatorAuthorizationRetryInvocation(
  ...
  if (creatorArgs.includes('--challenge-handoff')) {
    args.push('--challenge-handoff');
    for (const flag of ['--allow-custom-worker-url', '--no-open-browser']) {
      if (creatorArgs.includes(flag)) args.push(flag);
    }
  }
...
async function uploadBundleArtifact(
  client: TakuPublisherClient,
  directory: string,
  state: PublisherState,
  remoteId: string,
  ...
  const uploadUrl = responseValue(presigned, ['uploadUrl', 'upload_url'], true) as string;
  ...
  const reviewUrl = responseValue(completed, ['reviewUrl', 'review_url'], true) as string;
  ...
  state.listing = completedListing as JsonObject;
  state.updated_at = nowIso();
  await saveState(directory, state);
  return { artifact_id: artifactId, review_url: reviewUrl, response: completed };
}

export async function preserveRemoteListing(
  client: TakuPublisherClient,
  directory: string,
  state: PublisherState,
  remoteId: string,
  ...
  const saved = await client.updateDraft(remoteId, { listing: listing as JsonObject });
  const savedListing = extractDraftListing(saved) ?? listing as JsonObject;
...
function requiredListingFields(listing: JsonObject | null): string[] {
  if (!listing) return ['listing'];
  const missing: string[] = [];
  if (!String(listing.title ?? '').trim()) missing.push('title');
  if (!String(listing.description ?? '').trim()) missing.push('description');
  if (!String(listing.iconUrl ?? '').trim()) missing.push('iconUrl');
  return missing;
}

function changedListingFields(expected: JsonObject, actual: JsonObject | null): string[] {
  if (!actual) return ['listing'];
  return ['title', 'description', 'iconUrl'].filter(
    (field) => JSON.stringify(expected[field]) !== JSON.stringify(actual[field]),
  );
}

function assertReviewUrlDraft(reviewUrl: string, remoteId: string): void {
  let linkedDraftId = '';
  try {
  ...
  if (linkedDraftId !== remoteId) {
    throw new PublisherError(
      ...
      'remote_review_draft_mismatch',
      { remote_draft_id: remoteId },
    );
  }
}

async function localUploadReady(directory: string, state: PublisherState): Promise<boolean> {
  try {
    await verifyLocalBundle(directory, state);
    await buildPlatformScanPayload(directory, state);
    return true;
  } catch (error) {
    if (error instanceof PublisherError) return false;
    throw error;
  }
}

export function extractAssetIdentity(response: JsonObject): JsonObject | null {
  for (const candidate of responseCandidates(response)) {
    for (const key of ['assetIdentity', 'asset_identity']) {
      ...
      if (isRecord(identity) && typeof (identity.resourceId ?? identity.resource_id) === 'string' && String(identity.resourceId ?? identity.resource_id).trim()) return identity;
    }
  }
  return null;
}
...
function parseArguments(argv: string[]): ParsedArguments {
  ...
  if (positional.length) throw new PublisherError(`Unexpected positional arguments: ${positional.join(' ')}`, 'invalid_arguments');
...
function stringFlag(args: ParsedArguments, name: string, fallback: string): string {
...
function numberFlag(args: ParsedArguments, name: string, fallback: number): number {
  ...
  if (!Number.isFinite(numeric)) throw new PublisherError(`Argument --${name} must be a number.`, 'invalid_arguments');
  return numeric;
}
...
function publicUnit(unit: JsonObject): JsonObject {
  return {
    id: unit.id,
    type: unit.type,
    name: unit.name,
...
function requireRemoteId(state: PublisherState): string {
  const value = String(state.remote_draft_id ?? '').trim();
  if (!value) throw new PublisherError('Create the platform draft before using remote draft commands.', 'missing_remote_draft');
  return value;
}

function responseValue(response: JsonObject, keys: string[], required = false): string | null {
  for (const candidate of responseCandidates(response)) {
  ...
  if (required) throw new PublisherError(`Platform response is missing ${keys[0]}.`, 'invalid_api_response');
  return null;
}

function responseObject(response: JsonObject, keys: string[]): Record<string, string> {
  for (const candidate of responseCandidates(response)) {
    for (const key of keys) {
      const value = candidate[key];
      if (isRecord(value)) return Object.fromEntries(Object.entries(value).map(([name, child]) => [name, String(child)]));
    }
  }
  return {};
}

function iconGenerationPayload(payload: JsonObject, state: PublisherState): JsonObject {
  const listing = record(payload.listing);
...
function creatorArgument(args: string[], name: string): string | undefined {
  const flag = `--${name}`;
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index] as string;
    if (value.startsWith(`${flag}=`)) return value.slice(flag.length + 1).trim() || undefined;
    if (value === flag) return args[index + 1]?.trim() || undefined;
  }
  return undefined;
}
...
function creatorEnvironment(): NodeJS.ProcessEnv {
  ...
  for (const host of ['127.0.0.1', 'localhost', '::1']) if (!existing.includes(host)) existing.push(host);
  env.NO_PROXY = existing.join(',');
  env.no_proxy = env.NO_PROXY;
  return env;
}

async function readObject(file: string, message: string, code: string): Promise<JsonObject> {
  const value = await readJson(path.resolve(file));
  if (!isRecord(value)) throw new PublisherError(message, code);
  return value;
}
...
Commands:
  ...
  creator-draft --json --editor --challenge-handoff
  creator-challenge-select --draft <card-draft> --candidate-id <id>
  creator-challenge-status, creator-challenge-skip --draft <card-draft>
  creator-challenge-prepare --draft <card-draft> [--upload]
