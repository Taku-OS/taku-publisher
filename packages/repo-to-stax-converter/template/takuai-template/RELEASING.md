...
本仓库被 Taku 宿主用于 `create_project` 的默认 Taku App 模板来源。
...
模板仓库根目录包含维护模板本身所需的协作规则，但这些内部文件不能进入用户通过 `app_create` 创建的 Taku App。

- `.taku-template.json` 是生成产物的显式清单。
- `.agents`、`.claude`、`skills.md`、模板维护用文档、Changelog 与 release check 会在创建应用时移除。
- `.taku-template/payload/AGENTS.md` 与 `CLAUDE.md` 会覆盖到生成项目根目录，只包含通用 Taku App 开发约束。
- `.taku-template/payload/.agents/skills` 与 `.claude/skills` 提供相同的 Taku App 方法技能；`.agent-tools/superpowers/6.2.0` 是固定版本 fallback，并保留上游 MIT license。
- `package.json` 中仅供模板维护使用的 `release:check` script 会同步移除，避免留下失效命令。
- `pnpm run release:check` 会逐项校验 canonical `.taku-template.json` policy、用户版指南、标准 `SKILL.md` package、固定 Superpowers provenance 与 checker 内置的已审核 bundle SHA-256，以及内部负责人和内部 skill 文案没有进入 payload。额外的 exclude、cleanup、removeScripts，或只修改 bundle 并重算 payload manifest 摘要，都不会通过校验。
