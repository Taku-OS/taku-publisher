...
function runReleaseCheck(rootDir, tag = 'taku-3.0.4-template') {
...
test('Taku 3 template release contract is pinned to 0.3.4 and Node 20', () => {
  ...
  assert.match(packageJson.scripts['release:check'], /taku-3\.0\.4-template/);
...
test('only the repository default template must remain without runtime capability grants or QA UI', () => {
  const manifest = require('../taku.manifest.json');
  const rootPage = fs.readFileSync(path.join(ROOT_DIR, 'src/app/page.tsx'), 'utf8');
  const policy = require('../.taku-template.json');

  assert.equal(Object.hasOwn(manifest, 'runtimeCapabilities'), false);
  assert.equal(rootPage.includes('AgentRuntimeQaPanel'), false);
  assert.equal(policy.exclude.includes('scripts/template-release-contract.test.js'), true);
});

test('release check rejects runtime capabilities in the default template, not in generated apps', t => {
  const fixtureRoot = createReleaseFixture(t);
  const manifestFile = path.join(fixtureRoot, 'taku.manifest.json');
  ...
  assert.match(result.stderr, /默认模板不得声明 runtimeCapabilities/);
});

test('release check rejects reusing the previously published immutable template tag', t => {
  const fixtureRoot = createReleaseFixture(t);
  const result = runReleaseCheck(fixtureRoot, 'taku-3.0.3-template');

  assert.notEqual(result.status, 0, result.stdout);
  assert.match(result.stderr, /tag 必须是 taku-3\.0\.4-template/);
});
