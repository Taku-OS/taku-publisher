    ...
    for (const [index, line] of text.split(/\r?\n/).entries()) {
        ...
        if (!/\b(?:re\.compile|RegExp)\s*\(/.test(line)) {
            ...
            if (test(NON_LOOPBACK_PRIVATE_URL_PATTERN, line))
                findings.push(finding('private_network_url', 'block', relative, lineNumber, 'A local or private-network URL is not portable or public.', line));
...
function boundedFindings(findings) {
    ...
    if (!blockersTruncated)
        selected.push(...reviews.slice(0, capacity - selected.length));
    const omitted = findings.length - selected.length;
    selected.push(finding('finding_limit', blockersTruncated ? 'block' : 'review', '.', 0, `The scan generated ${findings.length} findings; ${omitted} lower-priority findings were omitted from this bounded report.`, ''));
    return selected;
}
