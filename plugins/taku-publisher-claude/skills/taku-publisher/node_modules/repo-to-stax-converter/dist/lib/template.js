...
export async function copySubAppTemplate(params) {
    ...
    if (policy)
        await applyPolicy(params.workspaceRoot, policy);
    const packageJson = await readJsonIfExists(join(params.workspaceRoot, 'package.json'));
...
async function readPolicy(templateRoot) {
    ...
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        throw new Error(`${POLICY_FILE} must be an object`);
    const record = raw;
    if (record.schemaVersion !== 1)
        throw new Error(`${POLICY_FILE} schemaVersion must be 1`);
    const exclude = parsePaths(record.exclude, 'exclude', true);
    const cleanup = parsePaths(record.cleanup, 'cleanup', false);
    const overlayDirectory = record.overlayDirectory === undefined
        ? undefined
    ...
    if (record.packageJson !== undefined) {
        ...
        const scripts = record.packageJson.removeScripts;
...
function parsePaths(value, label, required) {
    if (value === undefined && !required)
        return [];
    if (!Array.isArray(value))
        throw new Error(`${label} must be an array`);
    if (value.length > 128)
        throw new Error(`${label} exceeds the 128-path limit`);
...
function policyPath(workspaceRoot, relativePath) {
    const root = resolve(workspaceRoot);
    const target = resolve(root, normalize(relativePath));
    if (!target.startsWith(`${root}/`))
        throw new Error(`Template payload path escapes output: ${relativePath}`);
    return target;
}
async function applyPolicy(workspaceRoot, policy) {
    ...
    if (policy.overlayDirectory) {
        ...
        if (!(await pathExists(overlay)))
            throw new Error(`Template payload overlay does not exist: ${policy.overlayDirectory}`);
    ...
    if (policy.packageJson?.removeScripts.length) {
        ...
        if (!packageJson)
            throw new Error('Cannot apply template package policy: invalid package.json');
        if (packageJson.scripts && typeof packageJson.scripts === 'object' && !Array.isArray(packageJson.scripts)) {
            const scripts = packageJson.scripts;
            for (const name of policy.packageJson.removeScripts)
                delete scripts[name];
