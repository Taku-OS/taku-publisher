...
import { canonicalizePotentialPath, copyDirectory, pathExists, readJsonIfExists, safePackageName, writeJson, } from './fs.js';
import { routeRepoCapability } from './capability-router.js';
...
import { validateSubAppWorkspace } from './validator.js';
import { computeTreeDigest } from './tree-digest.js';
export async function convertRepoToStax(params) {
    ...
    if (route.kind !== 'subapp-migration') {
        throw new Error(`Repository is not eligible for SubApp conversion (${analysis.appType}): ${route.reason} ${route.nextAction}`);
    }
...
async function assertIndependentOutput(workspaceRoot, sourceRoot, templateRoot) {
...
async function patchBuildExcludes(workspaceRoot) {
    ...
    const existing = Array.isArray(tsconfig.exclude)
        ? tsconfig.exclude.filter((value) => typeof value === 'string')
        : [];
    const excludes = new Set(existing);
    excludes.add('upstream-source');
    excludes.add('upstream-source/**/*');
    tsconfig.exclude = [...excludes];
    await writeJson(tsconfigPath, tsconfig);
}
async function writeConversionPlan(workspaceRoot, analysis, name, sourceOmissions) {
...
async function writePlaceholderPage(workspaceRoot, analysis, name) {
...
async function removeTemplateDemoActions(workspaceRoot) {
