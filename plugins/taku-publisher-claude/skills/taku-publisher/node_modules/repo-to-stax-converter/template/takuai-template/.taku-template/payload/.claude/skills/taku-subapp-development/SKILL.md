...
For long-running generation, include a visible cancel action once an accepted `runId` is available. Disable repeat clicks synchronously, call `client.cancel(runId)`, and keep the original subscription until the Host confirms a terminal state. A cancel RPC receipt is not proof of cancellation; a failed request may retry cancellation of the same run, never create a replacement job. Late cancellation responses must not overwrite a completed result or a newer run. The executable example is `docs/examples/subapp-agent-runtime-qa/AgentRuntimeQaPanel.tsx` with its adjacent `run-cancellation.ts`; reuse only the cancellation wiring, not the QA interface.
