...
export async function resolveAuth(options = {}) {
    ...
    if (!session)
        return { token: '', source: 'missing', iconToken: '', scopes: [], sessionPath: desktopPath, refreshed: false };
    ...
    if (refreshed) {
        ...
        if (token)
            return { token, source: 'session', iconToken: token, scopes: [], sessionPath: desktopPath, refreshed: true };
    }
