...
import { assertTakuSubAppRuntimeManifest, SUBAPP_BUILD_ARCHIVE_FILE, SUBAPP_BUILD_OUTPUT_DIRECTORY, SUBAPP_SOURCE_ARCHIVE_FILE, } from '@taku/subapp-contract';
import { buildPublisherPackageArtifact } from './core.js';
import { checkSubAppConversion } from './subapp-agent.js';
import { readSubAppServiceAuthorizations, } from './subapp-services.js';
import { MAX_APP_STORE_PACKAGE_BYTES } from './constants.js';
import { atomicWriteBytes, atomicWriteJson, isRecord, PublisherError, sha256Bytes, } from './util.js';
...
export async function planSubAppPackage(request, options = {}) {
    const checked = await (options.checkConversion || checkSubAppConversion)(request.candidate, conversionOptions(options));
    if (!checked.converted) {
        throw new PublisherError('SubApp candidate must pass conversion validation before packaging.', 'subapp_package_conversion_gate_required');
    }
    ...
    if (receipt.candidateDigest !== checked.candidateDigest || receipt.ok !== true) {
        throw new PublisherError('Trusted runtime evidence is stale or belongs to different candidate content.', 'subapp_package_runtime_evidence_mismatch');
    }
    ...
    if (!buildSummary ||
        ...
        throw new PublisherError('Trusted preview build no longer matches its runtime evidence.', 'subapp_package_build_artifact_changed');
    }
    await assertRequiredBuildFiles(buildRoot);
    const runtimeManifest = await readRuntimeManifest(checked.workspaceRoot);
    const serviceAuthorizations = await readSubAppServiceAuthorizations(checked.workspaceRoot);
...
export async function packageSubApp(request, options = {}) {
    const plan = await planSubAppPackage({ candidate: request.candidate, runtimeEvidence: request.runtimeEvidence }, options);
    if (request.confirmationToken !== plan.confirmationToken) {
        throw new PublisherError('The SubApp package confirmation is missing, stale, or belongs to different evidence.', 'subapp_package_confirmation_mismatch');
    }
    const outputRoot = await canonicalExistingDirectory(request.outputRoot, 'package output');
    const packageName = normalizePackageName(request.name || String(plan.runtimeManifest.name ?? 'subapp'));
    const packageRoot = path.join(outputRoot, `${packageName}-release`);
    if (isWithin(packageRoot, plan.workspaceRoot) ||
        ...
        throw new PublisherError('SubApp package output must be separate from the candidate and runtime evidence.', 'subapp_package_output_overlaps_input');
    }
    if (await fs.lstat(packageRoot).catch(() => null)) {
        throw new PublisherError('SubApp package output already exists.', 'subapp_package_output_exists');
    }
    await fs.mkdir(packageRoot, { mode: 0o700 });
    try {
        const sourceEntries = await collectSourceEntries(plan.workspaceRoot);
        const buildEntries = await collectBuildEntries(path.join(plan.runtimeEvidenceRoot, BUILD_EVIDENCE_RELATIVE_PATH));
...
async function readRuntimeReceipt(evidenceRoot) {
    let value;
    try {
        value = JSON.parse(await fs.readFile(path.join(evidenceRoot, 'runtime-receipt.json'), 'utf8'));
    }
    catch {
        throw new PublisherError('Trusted runtime receipt is missing or invalid.', 'subapp_package_runtime_receipt_missing');
    }
    if (!isRecord(value) ||
        ...
        throw new PublisherError('Trusted runtime receipt is incompatible.', 'subapp_package_runtime_receipt_invalid');
    }
    return value;
}
function requireBuildArtifact(value) {
    if (!isRecord(value) ||
        ...
        throw new PublisherError('Trusted runtime build artifact record is incompatible.', 'subapp_package_build_artifact_invalid');
    }
    return value;
}
async function readRuntimeManifest(candidate) {
    try {
        const value = JSON.parse(await fs.readFile(path.join(candidate, 'taku.manifest.json'), 'utf8'));
        return assertTakuSubAppRuntimeManifest(value);
    }
    catch (error) {
        throw new PublisherError('SubApp runtime manifest is invalid.', 'subapp_package_manifest_invalid', { error: error instanceof Error ? error.message : String(error) });
    }
}
async function assertRuntimeScripts(candidate) {
    let packageJson;
    try {
    ...
    if (typeof scripts['start:preview'] !== 'string' ||
        !scripts['start:preview'].trim() ||
        typeof scripts['start:edit'] !== 'string' ||
        !scripts['start:edit'].trim()) {
        throw new PublisherError('SubApp package requires start:preview and start:edit scripts.', 'subapp_package_runtime_scripts_missing');
    }
    for (const command of [scripts['start:preview'], scripts['start:edit']]) {
        if (/(?:--port|-p)\s*=?\s*\d+|\bPORT\s*=\s*\d+/i.test(command)) {
            throw new PublisherError('SubApp runtime scripts must not hard-code a port.', 'subapp_package_hardcoded_port');
        }
    }
}
async function assertRequiredBuildFiles(buildRoot) {
    for (const required of REQUIRED_BUILD_FILES) {
        const metadata = await fs.lstat(path.join(buildRoot, required)).catch(() => null);
        if (!metadata?.isFile() || metadata.isSymbolicLink()) {
            throw new PublisherError(`Trusted preview build is missing ${required}.`, 'subapp_package_build_incomplete');
        }
    }
}
async function summarizeBuildTree(root) {
    ...
    async function walk(relativeRoot) {
        ...
        for (const child of children) {
            ...
            if (metadata.isSymbolicLink())
                throw new Error('build artifact contains a symlink');
            if (metadata.isDirectory()) {
                await walk(relativePath);
                continue;
            }
            if (!metadata.isFile())
                throw new Error('build artifact contains an unsupported entry');
            fileCount += 1;
...
async function collectSourceEntries(root) {
    ...
    return collectEntries(root, {
        ...
        include(relativePath, isDirectory) {
            ...
            if (parts.includes('.taku'))
                return false;
            if (isDirectory && SOURCE_EXCLUDED_DIRECTORIES.has(parts.at(-1) ?? ''))
                return false;
            ...
            if (isRequiredSourceEvidence(relativePath))
                return true;
            return isDirectory || !isGitIgnored(relativePath, gitIgnoreRules);
        },
    });
}
async function collectBuildEntries(root) {
    return collectEntries(root, {
...
function gitIgnoreGlobPattern(pattern) {
    let output = '';
...
function isGitIgnored(relativePath, rules) {
    let ignored = false;
...
function isRequiredSourceEvidence(relativePath) {
    if (relativePath.includes('/'))
        return false;
    return relativePath === 'UPSTREAM_CREDITS.md' || UPSTREAM_ATTRIBUTION_FILE.test(relativePath);
}
function assertAppStoreArtifactSize(size, label) {
    if (size <= MAX_APP_STORE_PACKAGE_BYTES)
        return;
    throw new PublisherError(`SubApp ${label} archive exceeds the ${MAX_APP_STORE_PACKAGE_BYTES} byte App Store limit.`, 'subapp_package_too_large', { archive: label, size, max_bytes: MAX_APP_STORE_PACKAGE_BYTES });
}
async function collectEntries(root, options) {
    ...
    async function walk(relativeRoot) {
        ...
        for (const child of children) {
            const relativePath = path.join(relativeRoot, child.name).replace(/\\/g, '/');
            if (!options.include(relativePath, child.isDirectory()))
                continue;
            const absolutePath = path.join(root, relativePath);
            const metadata = await fs.lstat(absolutePath);
            if (metadata.isSymbolicLink()) {
                throw new PublisherError(`SubApp package contains a symbolic link: ${relativePath}`, 'subapp_package_symlink');
            }
            if (metadata.isDirectory()) {
                await walk(relativePath);
                continue;
            ...
            if (!metadata.isFile()) {
                throw new PublisherError(`SubApp package contains an unsupported entry: ${relativePath}`, 'subapp_package_entry_unsupported');
            }
            totalBytes += metadata.size;
            if (entries.length + 1 > MAX_FILES || totalBytes > options.maxBytes) {
                throw new PublisherError('SubApp package exceeds the local file or size limit.', 'subapp_package_too_large');
            }
            entries.push({
...
async function canonicalEvidenceRoot(input, stateRoot) {
    const evidenceRoot = await canonicalExistingDirectory(input, 'runtime evidence');
    const allowedRoot = await fs.realpath(path.resolve(stateRoot ?? path.join(os.homedir(), '.taku', 'publisher', 'subapp-runtime')));
    if (!isWithin(evidenceRoot, path.join(allowedRoot, 'evidence'))) {
        throw new PublisherError('Runtime evidence is outside the Publisher trusted-runtime state.', 'subapp_package_evidence_outside_state');
    }
    return evidenceRoot;
}
...
async function verifyWrittenArtifact(filePath, expectedSha256, expectedSize) {
    const bytes = await fs.readFile(filePath);
    if (bytes.length !== expectedSize || sha256Bytes(bytes) !== expectedSha256) {
        throw new PublisherError('SubApp package failed its final integrity check.', 'subapp_package_integrity_failed');
    }
}
