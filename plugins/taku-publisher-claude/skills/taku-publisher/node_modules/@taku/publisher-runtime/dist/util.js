...
export class PublisherError extends Error {
    code;
    details;
    constructor(message, code = 'publisher_error', details = {}) {
        super(message);
...
export function jsonOutput(status, data = {}, options = {}) {
    const output = {
    ...
    if (options.actionType)
        output.action_type = options.actionType;
    return output;
}
export function emitJson(payload) {
    process.stdout.write(`${JSON.stringify(payload, null, 2)}\n`);
}
export async function secureDirectory(directory, mode = 0o700) {
    await fsp.mkdir(directory, { recursive: true, mode });
    await fsp.chmod(directory, mode).catch(() => undefined);
    return directory;
}
export async function atomicWriteBytes(file, data, mode = 0o600) {
...
export function draftDirectory(draftId, env = process.env) {
...
export async function loadState(draftId, env = process.env) {
...
export function ensureWithin(candidate, root, label = 'path') {
...
export function normalizedRelative(candidate, root) {
...
export async function walkTree(root, visitor, postOrder = false) {
    ...
    if (!postOrder)
        await visitor(root, 'directory');
    const entries = await fsp.readdir(root);
    entries.sort((a, b) => a.localeCompare(b));
    for (const name of entries)
        await walkTree(path.join(root, name), visitor, postOrder);
    if (postOrder)
        await visitor(root, 'directory');
}
