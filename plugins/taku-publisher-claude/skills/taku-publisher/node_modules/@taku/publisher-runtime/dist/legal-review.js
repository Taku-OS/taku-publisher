...
export function legalReviewAction(status, data, apiPath = '', siteUrl = process.env.TAKU_SITE_URL || DEFAULT_SITE_URL) {
    if (status !== 428 || !isRecord(data) || typeof data.error !== 'string' || !LEGAL_CODES.has(data.error))
        return null;
    ...
    if (!artifactReview && documents.length)
        url.searchParams.set('documents', documents.join(','));
...
export function publisherErrorOutput(error) {
