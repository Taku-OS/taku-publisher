...
import { readOpenCodeProjects, } from './opencode-local.js';
...
export async function discoverRecentProjects(options = {}) {
    ...
    const maxSessionFiles = boundedInteger(options.maxSessionFiles, DEFAULT_MAX_SESSION_FILES, 1, 2_000, 'maxSessionFiles');
    const env = options.env ?? process.env;
    const homeDir = path.resolve(options.homeDir ?? os.homedir());
    const codexHome = path.resolve(options.codexHome ?? env.CODEX_HOME ?? path.join(homeDir, '.codex'));
    const claudeConfigDir = path.resolve(options.claudeConfigDir ?? env.CLAUDE_CONFIG_DIR ?? path.join(homeDir, '.claude'));
    const cursorUserDir = path.resolve(options.cursorUserDir ?? resolveCursorUserDir(homeDir, options.platform ?? process.platform, env));
    const observations = [];
    for (const candidate of options.explicitProjects ?? []) {
        if (!path.isAbsolute(candidate)) {
            throw new PublisherError('Explicit projects must use absolute paths.', 'invalid_explicit_project');
        }
        observations.push({
    ...
    const sorted = [...projects.values()]
        .sort((left, right) => right.lastActivityMs - left.lastActivityMs || left.path.localeCompare(right.path))
        .slice(0, maxProjects);
    const output = [];
...
export function resolveCursorUserDir(homeDir, platform = process.platform, env = process.env) {
...
async function readCursorWorkspaceObservations(cursorUserDir, limit) {
...
function cursorWorkspacePath(value) {
...
async function collectSessionFiles(options) {
    ...
    if (options.host === 'all' || options.host === 'codex') {
        specs.push({ host: 'codex', root: path.join(options.codexHome, 'sessions'), maxDepth: 5 }, { host: 'codex', root: path.join(options.codexHome, 'archived_sessions'), maxDepth: 2 });
    }
    if (options.host === 'all' || options.host === 'claude-code') {
        specs.push({ host: 'claude-code', root: path.join(options.claudeConfigDir, 'projects'), maxDepth: 5 }, {
            host: 'claude-code',
...
async function collectJsonlFiles(root, maxDepth, host, output, exactFileName) {
    ...
    const visit = async (current, depth) => {
        ...
        for (const entry of entries) {
            ...
            if (!entry.isFile())
                continue;
            if (exactFileName ? entry.name !== exactFileName : !entry.name.endsWith('.jsonl'))
                continue;
            const fileStat = await fsp.stat(entryPath).catch(() => undefined);
            if (fileStat?.isFile())
                output.push({ host, path: entryPath, mtimeMs: fileStat.mtimeMs });
        }
    };
    await visit(root, 0);
}
async function readProjectObservation(file) {
    ...
    try {
        for await (const rawLine of lines) {
            ...
            if (lineCount > MAX_SESSION_LINES)
                break;
            if (!rawLine || Buffer.byteLength(rawLine, 'utf8') > MAX_SESSION_LINE_BYTES)
                continue;
            let value;
            try {
...
function findWorkspaceMetadata(value, depth = 0) {
    ...
    for (const [key, child] of Object.entries(value)) {
        ...
        if (WORKSPACE_KEYS.has(normalizedKey) &&
            typeof child === 'string' &&
...
function findActivityTimestamp(value, depth = 0) {
    ...
    for (const [key, child] of Object.entries(value)) {
        const normalizedKey = key.replace(/[^a-z0-9]/gi, '').toLowerCase();
        if (TIMESTAMP_KEYS.has(normalizedKey))
            latest = Math.max(latest, timestampMs(child));
        if (typeof child === 'object' && child !== null) {
...
async function safeWorkspaceDirectory(candidate, homeDir) {
...
async function inspectProjectMetadata(root) {
    const entries = await fsp.readdir(root, { withFileTypes: true }).catch(() => []);
    const names = new Set(entries.map((entry) => entry.name));
    const signals = [];
    if (names.has('SKILL.md'))
        signals.push('SKILL.md');
    ...
    if (signals.includes('SKILL.md'))
        routeHint = 'existing-skill';
    else if (hasAppFramework)
        routeHint = 'subapp-candidate';
    else if (signals.some((signal) => ['AGENTS.md', 'README.md', 'pyproject.toml', 'requirements.txt', 'package.json'].includes(signal)))
...
export function normalizeProjectHost(value) {
    const normalized = value.trim().toLowerCase();
    if (['all', 'codex', 'claude-code', 'cursor', 'opencode', 'other'].includes(normalized)) {
        return normalized;
    }
    if (normalized === 'claude' || normalized === 'cc')
        return 'claude-code';
    throw new PublisherError('Project host must be codex, claude-code, cursor, opencode, other, or all.', 'invalid_project_host');
}
