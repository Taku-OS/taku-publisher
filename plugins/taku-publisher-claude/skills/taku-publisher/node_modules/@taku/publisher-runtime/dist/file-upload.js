...
export function bufferedFileUploadTransport(transport) {
    return async (method, url, headers, file, timeoutMs) => {
        const { readFile } = await import('node:fs/promises');
        return transport(method, url, headers, await readFile(file), timeoutMs);
    };
}
export async function streamFileUploadTransport(method, url, headers, file, timeoutMs) {
    ...
    for (let redirects = 0; redirects <= 3; redirects += 1) {
        ...
        if (redirected.origin !== target.origin) {
            requestHeaders = Object.fromEntries(Object.entries(requestHeaders).filter(([name]) => !FORBIDDEN_REDIRECT_HEADERS.has(name.toLowerCase())));
        }
        target = redirected;
    }
    throw new Error('Upload redirect limit exceeded.');
}
async function sendFileRequest(method, target, headers, file, signal) {
    const proxy = proxyUrlFor(target, process.env);
    if (!proxy)
        return directFileRequest(method, target, headers, file, signal);
...
function directFileRequest(method, target, headers, file, signal) {
    const request = target.protocol === 'https:' ? https.request : http.request;
    return issueFileRequest(callback => request(target, { method, headers, signal }, callback), file);
}
function proxiedHttpFileRequest(method, target, proxy, headers, file, signal) {
    ...
    return issueFileRequest(callback => request({
...
async function tunneledHttpsFileRequest(method, target, proxy, headers, file, signal) {
    ...
    return issueFileRequest(callback => https.request({
        protocol: 'https:',
...
function connectProxy(target, proxy, signal) {
...
function issueFileRequest(createRequest, file) {
    return new Promise((resolve, reject) => {
...
function bypassProxy(target, value) {
