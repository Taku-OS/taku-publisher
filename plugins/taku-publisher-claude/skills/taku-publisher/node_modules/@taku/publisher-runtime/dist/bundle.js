...
import { canonicalCapabilityJson, createCapabilityPackageManifest, stableCapabilityId, } from '@taku/capability-contract';
...
import { atomicWriteBytes, atomicWriteJson, isRecord, nowIso, PublisherError, readJson, saveState, sha256Bytes, } from './util.js';
...
export async function buildBundle(directory, state, outputPath) {
    ...
    if (!outputPath && fs.existsSync(archivePath))
        throw new PublisherError('Bundle already exists for this draft.', 'bundle_exists');
    const files = Array.isArray(manifest.files) ? manifest.files.filter(isRecord) : [];
    const { bytes, bundleFiles } = await createReproducibleZip(path.join(directory, 'staging'), files, generated, state, requirements);
    ...
    if (!outputPath) {
        ...
        const fileList = { ...manifest, bundle_files: bundleFiles, bundle_file_count: bundleFiles.length };
...
function capabilityPackageManifest(state, files, requirements) {
    ...
    const requiredSecrets = Array.isArray(requirements.secrets)
        ? requirements.secrets
            ...
            .filter(Boolean)
        : [];
    const contentHash = sha256Bytes(Buffer.from(canonicalCapabilityJson({
...
export async function verifyLocalBundle(directory, state) {
    await assertStageUnchanged(directory, state);
    const bundlePath = path.join(directory, 'bundle.zip');
    if (!fs.existsSync(bundlePath))
        throw new PublisherError('Bundle has not been created.', 'missing_bundle');
    const actual = sha256Bytes(await fsp.readFile(bundlePath));
    if (actual !== state.bundle_sha256)
        throw new PublisherError('Bundle digest no longer matches local state.', 'bundle_changed');
    return bundlePath;
}
function bundleManifest(state) {
    const unit = isRecord(state.unit) ? state.unit : {};
...
async function createReproducibleZip(staging, stagedFiles, generated, state, requirements) {
    const entries = [];
    ...
    for (const [name, data] of generated)
        entries.push({ name, data, mode: 0o644 });
    ...
    const packageManifest = capabilityPackageManifest(state, packageFiles, requirements);
    entries.push({
...
function executableScript(relative) {
