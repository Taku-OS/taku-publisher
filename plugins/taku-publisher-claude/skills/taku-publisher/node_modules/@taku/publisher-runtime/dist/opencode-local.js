...
export function resolveOpenCodeDataDir(options = {}) {
    ...
    if ((options.platform ?? process.platform) === 'win32') {
        ...
        if (localAppData)
            return path.resolve(localAppData, 'opencode');
    }
    return path.join(homeDir, '.local', 'share', 'opencode');
}
export function openCodeStateDatabasePath(options = {}) {
    const env = options.env ?? process.env;
...
export async function readOpenCodeProjects(options = {}) {
...
export async function readOpenCodeStateUsage(options = {}) {
    const stateDbPath = openCodeStateDatabasePath(options);
...
export function openCodeProjectsFromRows(rows, source = 'opencode.db') {
    const projects = [];
    const seen = new Set();
    for (const row of Array.isArray(rows) ? rows : []) {
        const workspace = firstString(row.worktree, row.directory);
        if (!workspace || !path.isAbsolute(workspace) || seen.has(workspace))
            continue;
        seen.add(workspace);
        projects.push({
...
export function openCodeUsageSessionsFromRows(rows, fallbackTimestampMs = Date.now()) {
    const sessions = [];
    const seen = new Set();
    for (const row of Array.isArray(rows) ? rows : []) {
        ...
        if (Object.values(usage).reduce((sum, value) => sum + value, 0) <= 0)
            continue;
...
async function queryOpenCodeDatabase(databasePath, options) {
    ...
    const completed = await execFileAsync('sqlite3', ['-readonly', '-json', databasePath, sql], { encoding: 'utf8', timeout: options.timeoutMs, maxBuffer: MAX_SQLITE_OUTPUT_BYTES });
    const output = String(completed.stdout ?? '').trim();
    if (!output)
        return { rows: [], scannedByteCount: 0 };
...
async function readLegacyOpenCodeProjects(dataDir, limit) {
...
function openCodeModel(value) {
    if (isRecord(value))
        return firstString(value.id, value.modelID, value.model);
    if (typeof value !== 'string' || !value.trim())
        return '';
    try {
        const parsed = JSON.parse(value);
        if (isRecord(parsed))
            return firstString(parsed.id, parsed.modelID, parsed.model);
    }
    catch {
        return value.trim();
    }
