...
for (const host of SKILL_HOSTS) {
    ...
    for (const alias of host.aliases)
        HOST_BY_NAME.set(alias, host);
}
export function normalizeSkillHost(value) {
    const normalized = String(value ?? '').trim().toLowerCase();
    const host = HOST_BY_NAME.get(normalized);
    if (!host) {
        throw new PublisherError(`Skill host must be one of: ${SKILL_HOST_IDS.join(', ')}.`, 'unsupported_skill_host', { host: normalized || null });
    }
    return host.id;
}
export function skillHostDefinition(value) {
    const host = normalizeSkillHost(value);
    return SKILL_HOSTS.find((candidate) => candidate.id === host);
}
export function skillInstallRoot(value, options = {}) {
    ...
    if (hostHome === path.parse(hostHome).root
        || hostHome === homeDir
        || root === path.parse(root).root
        || root === homeDir
        || !isWithin(hostHome, root)) {
        throw new PublisherError(`${host.label} Skill install root is too broad.`, 'unsafe_install_target', { host: host.id });
    }
    return root;
}
