...
// @ts-nocheck
import { applyPersonaOverrides, personaSignatureFor, } from './persona.js';
import { ROOKIE_PERSONA_LOCALIZATIONS, personaLegacyArchetypes, } from './persona-catalog.js';
import { cleanText } from './privacy.js';
export { applyPersonaOverrides, buildPersonaIdentity, normalizePersonaOverrides, personaSignatureFor, publicHiddenPersona, publicTraitBadge, refreshPersonaIdentity, } from './persona.js';
...
function createEmptyBehaviorProfile(periodId = DEFAULT_USAGE_PERIOD_ID, periodLabel = 'Last 90 Days') {
    return {
...
export function createEmptyCreatorMetrics(options = {}) {
...
export function normalizeCreatorMetrics(input, options = {}) {
    ...
    const repoCount = readMetricNumber(github, [
        'repoCount',
        'repos',
        'repositoryCount',
        'totalRepoCount',
    ...
    normalized.percentiles = {
        tokens: readPercentileRank(percentiles, [
            'tokens',
            'token',
            'tokenUsage',
            'tokenPercentile',
        ]) || readPercentileRank(root, ['tokenPercentile', 'tokensPercentile']),
        stars: readPercentileRank(percentiles, [
            'stars',
            'githubStars',
            'starPercentile',
        ]) || readPercentileRank(github, ['starPercentile', 'starsPercentile']),
        installs: readPercentileRank(percentiles, [
            'installs',
            'skillInstalls',
            'installPercentile',
        ]) ||
        ...
        subscribers: readPercentileRank(percentiles, [
            'subscribers',
            'followers',
    ...
    normalized.topPercentiles = {
        tokens: readTopPercent(topPercentiles, [
            'tokens',
            'token',
            'tokenUsage',
            'tokensTopPercent',
        ]) || readTopPercent(root, ['tokenTopPercent', 'tokensTopPercent']),
        stars: readTopPercent(topPercentiles, [
            'stars',
            'githubStars',
            'starsTopPercent',
        ]) || readTopPercent(github, ['starsTopPercent', 'starTopPercent']),
        installs: readTopPercent(topPercentiles, [
            'installs',
            'skillInstalls',
            'installsTopPercent',
        ]) ||
        ...
        subscribers: readTopPercent(topPercentiles, [
            'subscribers',
            'followers',
    ...
    if (Array.isArray(root.warnings)) {
        normalized.warnings = Array.from(new Set([
            ...(normalized.warnings || []),
            ...root.warnings
...
export function mergeCreatorMetrics(...items) {
    const merged = createEmptyCreatorMetrics();
    const sourceSet = new Set();
    const warnings = [];
    for (const item of items) {
        const metrics = item?.schemaVersion === 'taku.creator.metrics.v1'
            ? item
            : normalizeCreatorMetrics(item);
        for (const source of metrics.sources || [])
            sourceSet.add(source);
        warnings.push(...(metrics.warnings || []));
        mergeMetricNumberFields(merged.taku, metrics.taku, [
            'skillInstallCount',
            'skillCount',
            'skillReferenceCount',
            'trustedCreatorReferenceCount',
        ...
        mergeMetricNumberFields(merged.github, metrics.github, [
            'repoCount',
            'publicRepoCount',
            'privateRepoCount',
            'totalStars',
        ...
        merged.taku.registrationRank = mergeRankMetric(merged.taku.registrationRank, metrics.taku?.registrationRank);
        merged.taku.activeInLast30Days = Boolean(merged.taku.activeInLast30Days || metrics.taku?.activeInLast30Days);
        merged.taku.betaTester = Boolean(merged.taku.betaTester || metrics.taku?.betaTester);
        mergeMetricNumberFields(merged.percentiles, metrics.percentiles, ['tokens', 'stars', 'installs', 'subscribers'], 'max');
        mergeMetricNumberFields(merged.topPercentiles, metrics.topPercentiles, ['tokens', 'stars', 'installs', 'subscribers'], 'min-positive');
...
export function metricTopPercent(metrics, key) {
...
export function isMetricTopPercent(metrics, key, maxTopPercent) {
    const topPercent = metricTopPercent(metrics, key);
    return topPercent > 0 && topPercent <= maxTopPercent;
}
export function topPercentEvidence(label, topPercent) {
...
function summarizeItemTypes(items) {
    ...
    return {
        ...counts,
        classifiedCount,
        makerShare: classifiedCount > 0
            ? round((counts.maker + counts.hybrid * 0.5) / classifiedCount, 3)
            : 0,
        infraShare: classifiedCount > 0
...
export function mergePersonaRules(base, override) {
    const merged = {
        ...base,
        ...asRecord(override),
        tonePacks: {
    ...
    if (!merged.defaultTone || !merged.tonePacks?.[merged.defaultTone])
        merged.defaultTone = base.defaultTone;
    return merged;
}
export function normalizePersonaTone(value, rules = DEFAULT_PERSONA_RULES) {
    ...
    if (!value)
        return tonePacks[defaultTone] ? defaultTone : Object.keys(tonePacks)[0] || 'brainrot';
...
export function composePersonaSignals({ usage, usedTools = [], ownedCreations = [], projectMetadata = {}, creatorMetrics = createEmptyCreatorMetrics(), includeGitHubMetrics = false, generatedAt, }) {
    ...
    const externalMetrics = mergeCreatorMetrics(creatorMetrics, normalizeCreatorMetrics({
...
export function buildPersonaV2(signals, options = {}) {
...
function derivePersonaTraits(signals, axes, rules = DEFAULT_PERSONA_RULES) {
    ...
    if (!Object.keys(traitRules || {}).length)
        return traits;
...
function deriveHiddenCandidates(signals, axes, rules = DEFAULT_PERSONA_RULES) {
    ...
    if (((external.taku?.registrationRank || 0) > 0 && external.taku.registrationRank <= 100) &&
