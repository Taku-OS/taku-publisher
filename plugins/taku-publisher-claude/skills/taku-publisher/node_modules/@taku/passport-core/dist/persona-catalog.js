export const PERSONA_PROFILE_SCHEMA_VERSION = 'taku.persona-profile.v1';
export const PERSONA_LOCALES = ['en-US', 'zh-CN'];
...
export const PERSONA_FAMILY_LOCALIZATIONS = {
...
export const PERSONA_CATALOG = Object.fromEntries(PERSONA_CODES.map((code) => {
...
export const ROOKIE_PERSONA_LOCALIZATIONS = {
    'en-US': {
        title: 'The Rookie',
...
export const PERSONA_TRAIT_AVATAR_KEYS = {
...
export const PERSONA_HIDDEN_AVATAR_KEYS = {
...
export const PERSONA_BADGE_LOCALIZATIONS = {
...
export function normalizePersonaLocale(value) {
    return String(value || '').toLowerCase().startsWith('zh') ? 'zh-CN' : 'en-US';
}
export function isPersonaCode(value) {
    return PERSONA_CODES.includes(String(value || '').toUpperCase());
}
export function personaFamilyForCode(codeValue) {
...
export function personaAxisTagsForCode(codeValue) {
...
export function personaLegacyArchetypes(localeValue = 'zh-CN') {
    const locale = normalizePersonaLocale(localeValue);
    return Object.fromEntries(PERSONA_CODES.map((code) => {
...
export function personaDescriptionFor(codeValue, localeValue = 'zh-CN') {
    const code = String(codeValue || '').toUpperCase();
    if (!isPersonaCode(code)) {
        return code === 'ROOKIE'
    ...
    return PERSONA_CATALOG[code].localizations[normalizePersonaLocale(localeValue)].description;
}
//# sourceMappingURL=persona-catalog.js.map