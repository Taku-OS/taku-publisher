...
function asPersonaRecord(value) {
...
export function personaSignatureFor(code, tone, archetype = {}) {
    const explicit = cleanText(archetype.signature, 220);
    if (explicit)
        return explicit;
    const normalizedCode = String(code || '');
    if (normalizeChoiceToken(tone) === 'brainrot') {
        const catalogDescription = personaDescriptionFor(normalizedCode, 'zh-CN');
...
export function normalizePersonaOverrides(overridesValue = {}, personaValue = {}) {
    const overrides = asPersonaRecord(overridesValue);
    const persona = asPersonaRecord(personaValue);
    const availableToneIds = new Set((Array.isArray(persona.availableTones) ? persona.availableTones : [])
        .map((tone) => asPersonaRecord(tone).id));
    const manualTraitIds = new Set((Array.isArray(persona.manualTraitCatalog)
        ? persona.manualTraitCatalog
        : []).map((trait) => asPersonaRecord(trait).id));
    const autoTraitIds = new Set((Array.isArray(persona.autoTraits)
        ? persona.autoTraits
        : Array.isArray(persona.traits)
            ? persona.traits
            : []).map((trait) => asPersonaRecord(trait).id));
    const hiddenIds = new Set((Array.isArray(persona.hiddenCandidates)
        ? persona.hiddenCandidates
        : []).map((item) => asPersonaRecord(item).id));
    const tone = typeof overrides.tone === 'string' &&
        availableToneIds.has(overrides.tone)
        ? overrides.tone
        : undefined;
    const lockedCode = typeof overrides.lockedCode === 'string' &&
        /^[AE][MI][OL][HW]$/.test(overrides.lockedCode)
        ? overrides.lockedCode
        : undefined;
    const hiddenTraitIds = Array.isArray(overrides.hiddenTraitIds)
        ? uniqueStrings(overrides.hiddenTraitIds.filter((id) => autoTraitIds.has(id)))
        : [];
    const addedTraitIds = Array.isArray(overrides.addedTraitIds)
        ? uniqueStrings(overrides.addedTraitIds.filter((id) => manualTraitIds.has(id)))
        : [];
    const selectedHiddenId = typeof overrides.selectedHiddenId === 'string' &&
...
function selectHiddenArchetype(persona, selectedHiddenId) {
    if (!selectedHiddenId)
        return undefined;
    return (Array.isArray(persona.hiddenCandidates) ? persona.hiddenCandidates : []).find((item) => asPersonaRecord(item).id === selectedHiddenId);
}
function hiddenPriorityIndex(id) {
    const index = HIDDEN_PERSONA_PRIORITY.indexOf(String(id || ''));
    return index >= 0 ? index : HIDDEN_PERSONA_PRIORITY.length;
}
function selectFeaturedHiddenArchetype(candidatesValue = []) {
    ...
    return [...normalized].sort((left, right) => {
        ...
        const priorityDelta = hiddenPriorityIndex(leftRecord.id) -
            hiddenPriorityIndex(rightRecord.id);
        if (priorityDelta !== 0)
            return priorityDelta;
        return ((Number(rightRecord.confidence) || 0) -
            (Number(leftRecord.confidence) || 0));
    })[0];
}
export function publicHiddenPersona(itemValue) {
    if (!itemValue)
        return undefined;
...
export function publicTraitBadge(traitValue) {
    if (!traitValue)
        return undefined;
...
export function buildPersonaIdentity(personaValue = {}) {
    const persona = asPersonaRecord(personaValue);
    const unlockedHidden = (Array.isArray(persona.hiddenCandidates) ? persona.hiddenCandidates : [])
        .map(publicHiddenPersona)
        .filter(Boolean);
    const hidden = asPersonaRecord(persona.hidden);
    const featuredHidden = publicHiddenPersona(hidden.featured ||
        persona.selectedHidden ||
        selectFeaturedHiddenArchetype(persona.hiddenCandidates));
    const badges = (Array.isArray(persona.traits) ? persona.traits : [])
    ...
    return {
        basePersona: {
            ...
            signature: cleanText(archetype.signature, 220) ||
...
export function refreshPersonaIdentity(personaValue = {}) {
    const next = structuredClone(asPersonaRecord(personaValue));
    const unlocked = (Array.isArray(next.hiddenCandidates) ? next.hiddenCandidates : []).filter((item) => asPersonaRecord(item).id);
    const featured = next.selectedHidden || selectFeaturedHiddenArchetype(unlocked);
    next.hidden = {
        unlocked,
        ...(featured ? { featured } : {}),
...
export function applyPersonaOverrides(personaValue, overridesValue = {}) {
    if (!personaValue)
        return personaValue;
    const persona = asPersonaRecord(personaValue);
    ...
    const baseTraits = (Array.isArray(next.autoTraits)
        ? next.autoTraits
        : Array.isArray(next.traits)
            ? next.traits
            : []).filter((trait) => !hiddenTraitIds.has(asPersonaRecord(trait).id));
    const manualCatalog = new Map((Array.isArray(next.manualTraitCatalog)
    ...
    next.selectedHidden = selectHiddenArchetype(next, normalized.selectedHiddenId);
