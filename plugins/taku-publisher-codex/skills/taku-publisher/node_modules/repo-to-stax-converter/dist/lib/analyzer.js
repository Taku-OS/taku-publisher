...
const BUNDLED_TEMPLATE_ROOT = fileURLToPath(new URL('../../template/takuai-template/', import.meta.url));
...
export async function analyzeRepo(params) {
    ...
    const rootPackage = await readJsonIfExists(join(repoRoot, 'package.json'));
    const frontendPackage = await readJsonIfExists(join(repoRoot, 'frontend', 'package.json'));
    const browserManifest = await readJsonIfExists(join(repoRoot, 'manifest.json'));
    const packageJson = rootPackage ?? frontendPackage ?? null;
    const files = await listFiles(repoRoot);
    const contentSignals = await detectRepoContentSignals(repoRoot, files, params.templateBaselineRoot === undefined
...
function detectAppType(input) {
    ...
    const hasFastApi = input.contentSignals.hasFastApi || (input.files.some(file => file.endsWith('.py')) && filesContain(input.files, ['fastapi']));
    ...
    const hasStandaloneSkill = fileSet.has('SKILL.md') || input.files.some(file => /(?:^|\/)skills\/[^/]+\/SKILL\.md$/.test(file));
    ...
    const hasExternalConnector = input.contentSignals.hasExternalConnector &&
    ...
    if (hasGradio)
        return 'gradio';
    if (hasSkill || hasStandaloneSkill)
        return 'workflow-skill';
    if (input.files.some(file => file.endsWith('.py')))
        return 'python-cli';
    return 'unknown';
}
async function detectRepoContentSignals(repoRoot, files, templateBaselineRoot) {
    const signalTextParts = [];
    const externalServiceTextParts = [];
    ...
    for (const file of signalFiles) {
        ...
        if (text) {
            ...
            if (isExternalServiceSignalFile(file) &&
    ...
    return {
        ...
        hasPrivateDocumentDomain: /\b(resume|pdf|upload|embedding|faiss|transcript|xlsx|csv)\b/.test(signalText) ||
        ...
        hasExternalConnector: /\b(?:daemon|webhook|oauth|bot|bridge|qr code|long-lived service)\b/.test(signalText),
    };
}
async function matchesBundledTemplateFile(templateBaselineRoot, relativePath, sourceText) {
    if (!templateBaselineRoot)
        return false;
...
function isExternalServiceSignalFile(file) {
    const lower = file.toLowerCase();
    if (/(?:^|\/)(?:docs?|examples?|test|tests|__tests__)(?:\/|$)/.test(lower) ||
        /\.(?:test|spec)\.[^.]+$/.test(lower) ||
        /(?:^|\/)(?:readme|changelog|releasing)(?:\.[^/]*)?$/.test(lower)) {
        return false;
    }
    return (/\.(?:cjs|go|java|js|jsx|json|kt|mjs|php|py|rb|rs|ts|tsx)$/.test(lower) ||
        /(?:^|\/)\.env(?:\.|$)/.test(lower) ||
        lower.includes('requirements') ||
        lower.includes('dockerfile'));
}
function detectExternalServiceEvidence(text) {
    if (!text)
        return false;
    const credentialUsage = /process\.env(?:\.|\[['"])[A-Z0-9_]*(?:API_?KEY|ACCESS_?TOKEN|AUTH_?TOKEN|SECRET|CLIENT_SECRET)\b/i.test(text) ||
    ...
    if (credentialUsage)
        return true;
    const externalSdkDependency = /["'](?:@anthropic-ai\/sdk|@google\/generative-ai|openai|stripe|replicate|twilio|@aws-sdk\/[^"']+)["']\s*:/i.test(text);
    if (externalSdkDependency)
        return true;
    const requestPattern = /(?:fetch|axios\.(?:get|post|put|patch|delete)|requests\.(?:get|post|put|patch|delete)|httpx\.(?:get|post|put|patch|delete))\s*\(\s*[`'"](https?:\/\/[^`'"\s]+)/gi;
...
async function detectLicense(root) {
    const text = (await readTextIfExists(join(root, 'LICENSE'))) ??
    ...
    if (normalized.includes('mit license'))
        return 'MIT';
    if (normalized.includes('gnu affero'))
        return 'AGPL-3.0';
...
function buildReasons(appType, hasUi, hasReadme, license) {
    const reasons = [];
    const labels = {
    ...
    if (hasUi)
        reasons.push('Interactive UI surface detected');
    if (hasReadme)
        reasons.push('README present for product extraction');
    if (license !== 'Unknown')
        reasons.push(`License detected: ${license}`);
    return reasons;
}
function buildRisks(appType, files, pkg, signals, license) {
    const risks = [];
    if (appType === 'unknown')
        risks.push('Unknown framework requires manual review');
    if (appType === 'workflow-skill')
        risks.push('Workflow repo needs a new UI wrapper');
    if (appType === 'browser-extension')
        risks.push('Browser extensions are reference-only in Taku Stax');
    if (appType === 'external-connector')
        risks.push('External connectors require an independently managed runtime');
    if (appType === 'streamlit' || appType === 'gradio')
        risks.push('Python UI must be ported to Next.js');
    if (license === 'Unknown')
        risks.push('Unknown license blocks publish until review');
    if (files.some(file => file.toLowerCase().includes('docker-compose')))
        risks.push('Docker compose dependency may require manual extraction');
    const deps = dependenciesOf(pkg).join(' ').toLowerCase();
    if (deps.includes('cuda') || deps.includes('torch'))
        risks.push('Heavy ML dependencies may not fit a local SubApp');
    if (signals.hasTradingDomain)
        risks.push('Financial or trading workflow needs no-advice/no-live-orders safety review');
    if (signals.hasPrivateDocumentDomain)
        risks.push('Document or upload workflow may handle sensitive user data');
    if (signals.hasExternalServiceEvidence)
        risks.push(CREDENTIAL_RISK);
    if (signals.hasDangerousCommandText)
        risks.push('Potential command execution requires security review before conversion');
    return risks;
}
function buildServiceRequirements(signals) {
    if (!signals.hasExternalServiceEvidence)
        return [];
    return [
        {
            ...
            mapping: {
                ...
                reason: 'The Converter detected executable external-service usage. Map the exact operation to current Taku service catalog IDs before conversion.',
            },
        },
    ];
}
function scoreRepo(appType, hasUi, hasReadme, license, risks) {
    let score = 20;
    if (appType === 'nextjs')
        score += 55;
    else if (appType === 'fastapi-next')
        score += 50;
    else if (appType === 'vite-react')
        score += 45;
    else if (appType === 'streamlit' || appType === 'gradio')
        score += 35;
    else if (appType === 'workflow-skill')
        score += 30;
    else if (appType === 'browser-extension' || appType === 'external-connector')
        score += 5;
    else if (appType === 'python-cli')
        score += 15;
    if (hasUi)
        score += 15;
    if (hasReadme)
