...
export async function prepareTemplateSource(params) {
    ...
    const target = join(boundaryRoot, '.template-source-cache', safePackageName(`${slug}-${params.ref}`), 'repo');
...
export async function refreshImmutableGitRef(params) {
    ...
    if (origin !== params.url)
        throw new Error(`Template cache origin mismatch for ${target}`);
    ...
    if (!commit)
        throw new Error(`Cannot resolve immutable template ref ${params.ref}`);
    await assertOwnedGitCache({ target, boundaryRoot: params.boundaryRoot, kind: 'template-source', url: params.url });
