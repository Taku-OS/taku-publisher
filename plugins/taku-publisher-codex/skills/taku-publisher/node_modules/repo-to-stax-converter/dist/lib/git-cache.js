...
export async function assertOwnedGitCache(params) {
    ...
    const targetMetadata = await lstat(target).catch(() => null);
    if (!targetMetadata)
        throw new Error(`Owned Git cache checkout is missing: ${target}`);
...
function assertNestedInBoundary(boundaryRoot, target) {
...
async function assertNoSymlinkChain(boundaryRoot, targetRoot) {
    ...
    for (const segment of lexicalRelative.split(/[\\/]/)) {
        ...
        if (!metadata)
            throw new Error(`Git cache ancestor is missing: ${cursor}`);
        if (metadata.isSymbolicLink()) {
            throw new Error(`Refusing symbolic link in Git cache ancestor chain: ${cursor}`);
        }
        if (!metadata.isDirectory())
            throw new Error(`Git cache ancestor is not a directory: ${cursor}`);
    }
...
async function assertRealDirectory(path, label) {
    ...
    if (metadata.isSymbolicLink())
        throw new Error(`Refusing symbolic link as ${label}: ${path}`);
    if (!metadata.isDirectory())
        throw new Error(`${label} is not a directory: ${path}`);
}
async function readAndValidateSentinel(path, kind, url) {
    const metadata = await lstat(path).catch(() => null);
    if (!metadata)
        throw new Error(`Git cache ownership sentinel is missing: ${path}`);
