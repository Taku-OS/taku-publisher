...
export { assertRuntimeCapabilities } from './runtime-capabilities.js';
export const SUBAPP_CONTRACT_VERSION = '0.1.0';
export const SUBAPP_ASSESSMENT_SCHEMA_VERSION = 'taku.subapp-assessment.v1';
export const SUBAPP_MIGRATION_SCHEMA_VERSION = 'taku.subapp-migration.v2';
export const SUBAPP_VALIDATION_SCHEMA_VERSION = 'taku.subapp-validation.v1';
export const SUBAPP_RELEASE_SCHEMA_VERSION = 'taku.subapp-release.v1';
export const SUBAPP_RUNTIME_MANIFEST_FILE = 'taku.manifest.json';
export const SUBAPP_MIGRATION_RECORD_FILE = '.taku/migration.json';
export const SUBAPP_SOURCE_ARCHIVE_FILE = 'source.zip';
export const SUBAPP_BUILD_ARCHIVE_FILE = 'build.zip';
export const SUBAPP_BUILD_OUTPUT_DIRECTORY = '.next-preview';
...
const RELATIVE_PATH_PATTERN = /^(?!\/)(?![A-Za-z]:[\\/])(?!.*(?:^|\/)\.\.(?:\/|$))[^\0]+$/;
const PRIVATE_PATH_PATTERN = /(?:^|[\s"'`(])(?:\/Users|\/home|\/private\/var\/folders|\/var\/folders|\/Volumes)\/[^\s"'`)]+|[A-Za-z]:\\(?:Users|Documents and Settings)\\/;
const FILE_URL_PATTERN = /file:\/\/\//i;
const PRIVATE_KEY_PATTERN = /-----BEGIN (?:[A-Z0-9 ]+ )?PRIVATE KEY-----/;
const KNOWN_TOKEN_PATTERN = /\b(?:sk-(?:proj-)?[A-Za-z0-9_-]{16,}|github_pat_[A-Za-z0-9_]{20,}|gh[pousr]_[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]{16,}|glpat-[A-Za-z0-9_-]{16,}|npm_[A-Za-z0-9]{20,})\b/;
...
export function createSubAppAssessment(input) {
    ...
    const findings = normalizeFindings(input.findings, input.analysis.risks, serviceRequirements);
    const eligibility = eligibilityFor(input.analysis, input.route, serviceRequirements, findings);
    ...
    if (eligibility === 'rejected' && !findings.some((finding) => finding.severity === 'blocker')) {
        findings.push({
            ...
            category: 'product',
            severity: 'blocker',
            message: input.route.kind === 'subapp-migration'
...
export function assertSubAppAssessment(value) {
    ...
    if (route.capability !== analysis.appType) {
        throw new TypeError('Assessment route capability must match the detected appType.');
    }
    const expectedRoute = routeKindForProjectType(analysis.appType);
    if (route.kind !== expectedRoute) {
        throw new TypeError(`Assessment route for ${analysis.appType} must be ${expectedRoute}.`);
    }
    const serviceRequirements = requireArray(assessment.serviceRequirements, 'assessment serviceRequirements').map(assertServiceRequirement);
    ...
    for (const finding of findings)
        assertAssessmentFinding(finding);
    const eligibility = requireEnum(assessment.eligibility, SUBAPP_ELIGIBILITY, 'assessment eligibility');
    const expectedEligibility = eligibilityFor(analysis, route, serviceRequirements, findings);
...
export function assertSubAppMigrationRecord(value) {
    ...
    for (const key of ['url', 'repo', 'license'])
        requireText(source[key], `migration source ${key}`);
    ...
    for (const key of [
        ...
        requirePrefixedSha256(source[key], `migration ${key}`);
    ...
    for (const key of ['requestedRef', 'resolvedRef', 'commit', 'version']) {
        optionalNullableText(template[key], `migration template ${key}`);
    }
    if (template.policyApplied !== true)
        throw new TypeError('Migration template policy must be applied.');
...
export function assertSubAppValidationResult(value) {
    ...
    if (typeof result.ok !== 'boolean')
        throw new TypeError('Validation ok must be boolean.');
    ...
    for (const item of findings) {
        ...
        if (finding.path !== undefined)
            requireRelativePath(finding.path, 'validation path');
    }
...
export function assertTakuSubAppRuntimeManifest(value) {
    ...
    if (manifest.description !== undefined)
        requireText(manifest.description, 'runtime manifest description', true);
    if (manifest.version !== undefined)
        requireText(manifest.version, 'runtime manifest version');
    if (manifest.iconPath !== undefined)
        requireRelativePath(manifest.iconPath, 'runtime manifest iconPath');
    if (manifest.actions !== undefined) {
        ...
        for (const item of actions) {
            ...
            if (names.has(action.name))
                throw new TypeError(`Duplicate SubApp Action name: ${action.name}.`);
    ...
    if (manifest.llm !== undefined) {
        ...
        for (const key of ['required', 'preferLocal']) {
            if (llm[key] !== undefined && typeof llm[key] !== 'boolean') {
                throw new TypeError(`Runtime manifest llm ${key} must be boolean.`);
            }
        }
        if (llm.fallback !== undefined)
            requireEnum(llm.fallback, ['cloud', 'none'], 'runtime manifest llm fallback');
    }
    return value;
}
export function assertSubAppRelease(value) {
    ...
    const serviceAuthorizations = publish.serviceAuthorizations === undefined
        ? []
        : requireArray(publish.serviceAuthorizations, 'publish serviceAuthorizations');
    const seenServices = new Set();
    for (const value of serviceAuthorizations) {
        const authorization = requireRecord(value, 'publish service authorization');
        assertExactKeys(authorization, ['serviceId', 'endpointIds'], 'publish service authorization');
        const serviceId = requireCatalogId(authorization.serviceId, 'publish service authorization serviceId');
        ...
        const endpointIds = requireStringArray(authorization.endpointIds, 'publish service authorization endpointIds');
        ...
        endpointIds.forEach(endpointId => requireCatalogId(endpointId, 'publish service authorization endpointId'));
    }
    return value;
}
export function canonicalSubAppJson(value) {
    return JSON.stringify(sortJson(value));
}
export function hashCanonicalSubApp(value) {
    return createHash('sha256').update(canonicalSubAppJson(value)).digest('hex');
}
export function assertPublicSubAppValue(value) {
    inspectPublicValue(value, '$');
}
function eligibilityFor(analysis, route, services, findings) {
    if (route.kind !== 'subapp-migration' || analysis.recommendation === 'not-recommended')
        return 'rejected';
    if (findings.some((finding) => finding.severity === 'blocker')) {
        return 'rejected';
    }
    if (services.some((service) => service.required && service.mapping.status === 'unavailable')) {
        return 'rejected';
    }
    if (analysis.recommendation === 'manual-review' ||
        findings.some((finding) => finding.severity === 'warning') ||
        services.some((service) => service.required && service.mapping.status === 'review-required')) {
        return 'review-required';
    }
    return 'eligible';
}
function routeKindForProjectType(appType) {
    if (['nextjs', 'vite-react', 'fastapi-next', 'streamlit', 'gradio'].includes(appType)) {
        return 'subapp-migration';
    }
    return appType === 'workflow-skill' ? 'native-import' : 'reference-only';
}
function nextStepFor(eligibility, route) {
...
function normalizeFindings(provided, risks, services) {
    ...
    for (const service of services) {
        if (!service.required || service.mapping.status === 'mapped')
            continue;
        const message = service.mapping.reason ||
            `Required service capability "${service.capability}" is not mapped to Taku.`;
        if (messages.has(message))
            continue;
        findings.push({
            code: service.mapping.status === 'unavailable'
                ? 'service.mapping-unavailable'
                : 'service.mapping-review-required',
            category: 'runtime',
            severity: service.mapping.status === 'unavailable' ? 'blocker' : 'warning',
            message,
        });
...
function cloneServiceRequirement(input) {
    return {
        ...input,
...
function assertServiceRequirement(value) {
    ...
    const operations = requireStringArray(service.operations, 'service requirement operations');
    ...
    assertExactKeys(mapping, ['status', 'serviceId', 'endpointIds', 'reason'], 'service requirement mapping');
    const status = requireEnum(mapping.status, ['mapped', 'review-required', 'unavailable'], 'service mapping status');
    if (mapping.serviceId !== undefined) {
        requireCatalogId(mapping.serviceId, 'service mapping serviceId');
    }
    const endpointIds = mapping.endpointIds === undefined
        ? []
        : requireStringArray(mapping.endpointIds, 'service mapping endpointIds');
    endpointIds.forEach((endpointId) => requireCatalogId(endpointId, 'service mapping endpointId'));
    ...
    if (status === 'mapped' && (!mapping.serviceId || endpointIds.length === 0)) {
        throw new TypeError('Mapped services require a Taku serviceId and at least one endpointId.');
    }
    if (status !== 'mapped' && !cleanText(mapping.reason, 2000)) {
        throw new TypeError('Unmapped or review-required services require a reason.');
    }
    return service;
}
...
function assertAssessmentFinding(value) {
    ...
    if (finding.path !== undefined)
        requireRelativePath(finding.path, 'assessment finding path');
}
function assertMigrationAnalysis(value) {
    ...
    for (const item of resolutions) {
        ...
        if (!risks.includes(risk))
            throw new TypeError('Migration risk resolution must reference an exact recorded risk.');
...
function assertAction(value) {
    ...
    if (!/^[A-Za-z][A-Za-z0-9._-]{0,127}$/.test(name))
        throw new TypeError(`Invalid SubApp Action name: ${name}.`);
    if (action.description !== undefined)
        requireText(action.description, 'Action description', true);
    if (action.runtime !== undefined)
        requireEnum(action.runtime, ['server_http', 'client_postmessage'], 'Action runtime');
    if (action.params !== undefined) {
        const params = requireRecord(action.params, 'Action params');
        for (const [paramName, input] of Object.entries(params)) {
            if (!/^[A-Za-z][A-Za-z0-9_-]{0,127}$/.test(paramName))
                throw new TypeError(`Invalid Action parameter name: ${paramName}.`);
            ...
            if (param.description !== undefined)
                requireText(param.description, `Action parameter ${paramName} description`, true);
            if (param.required !== undefined && typeof param.required !== 'boolean')
                throw new TypeError(`Action parameter ${paramName} required must be boolean.`);
            if (param.enum !== undefined && !Array.isArray(param.enum))
                throw new TypeError(`Action parameter ${paramName} enum must be an array.`);
            if (param.format !== undefined)
                requireText(param.format, `Action parameter ${paramName} format`);
            if (param.ui !== undefined)
                assertActionUi(param.ui, paramName);
        }
    }
    if (action.returns !== undefined) {
        ...
        if (returns.type !== undefined)
            requireText(returns.type, 'Action returns type');
        if (returns.description !== undefined)
            requireText(returns.description, 'Action returns description', true);
    }
    if (action.execution !== undefined) {
        ...
        if (execution.defaultMode !== undefined)
            requireEnum(execution.defaultMode, ['sync', 'async'], 'Action defaultMode');
        if (execution.timeoutMs !== undefined)
            requirePositiveInteger(execution.timeoutMs, 'Action timeoutMs');
    }
...
function assertActionUi(value, paramName) {
    ...
    if (ui.label !== undefined)
        requireText(ui.label, 'Action UI label');
    if (ui.placeholder !== undefined)
        requireText(ui.placeholder, 'Action UI placeholder', true);
    if (ui.widget !== undefined)
        requireEnum(ui.widget, ['text', 'textarea', 'select', 'number', 'switch', 'filePicker', 'pathPicker'], 'Action UI widget');
}
function assertReleaseArtifact(value, fileName) {
...
function assertSourceRights(value) {
    ...
    if (authorship === 'original') {
        if (basis !== 'self_owned')
            throw new TypeError('Original SubApps require self_owned rights.');
        if (rights.sourceUrl !== '')
            requirePublicHttpsUrl(rights.sourceUrl, 'original source URL');
        return;
    }
    requirePublicHttpsUrl(rights.sourceUrl, 'derived source URL');
    if (basis === 'self_owned')
        throw new TypeError('Derived and third-party SubApps cannot use self_owned rights.');
...
function inspectPublicValue(value, path) {
    ...
    for (const [key, child] of Object.entries(value)) {
        if (FORBIDDEN_PUBLIC_KEYS.has(key))
            throw new TypeError(`Public SubApp release contains forbidden field ${key} at ${path}.`);
...
function optionalNullableText(value, label) {
...
function requireScore(value) {
    if (!Number.isInteger(value) || value < 0 || value > 100)
        throw new TypeError('Analysis score must be an integer from 0 to 100.');
}
...
function requirePrefixedSha256(value, label) {
...
function requireRelativePath(value, label) {
    ...
    if (!RELATIVE_PATH_PATTERN.test(path) || path.includes('\\'))
        throw new TypeError(`${label} must be a safe POSIX relative path.`);
    return path;
}
...
function requireCatalogId(value, label) {
