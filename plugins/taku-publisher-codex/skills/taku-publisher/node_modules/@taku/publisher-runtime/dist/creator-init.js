import { authStatus, resolveAuth, } from './auth.js';
...
export async function initializeCreator(options = {}, dependencies = {}) {
    ...
    const [auth, projects, editorResult] = await Promise.all([
        ...
        dependencies.createEditor
            ? dependencies.createEditor().then((value) => ({ value, warning: null }), (error) => ({
                value: {},
    ...
    return {
        ...
        projects: projects,
        projectChoices: projects.map(creatorProjectChoice),
...
async function fetchCreatorProfile(auth, options) {
    if (!auth.token)
        return {};
...
function findString(value, keys, depth = 0) {
    ...
    for (const key of keys) {
        ...
        if (typeof child === 'string' && child.trim())
            return child.trim().replace(/^@+/, '').slice(0, 80);
    }
...
function buildProfileEntryUrl(siteUrl, intent) {
...
export function creatorInitProjects(value) {
