...
import { resolveTakuServiceCatalogUrl, } from './service-catalog.js';
export const TAKU_SERVICE_SEARCH_SCHEMA_VERSION = 'taku.service-search.v1';
...
export function resolveTakuServiceSearchUrl(catalogUrl) {
    ...
    const trustedCatalogUrl = new URL(resolveTakuServiceCatalogUrl(normalizedUrl, {}));
...
export async function fetchTakuServiceSearch(request) {
    ...
    try {
        ...
        throw new PublisherError(controller.signal.aborted
            ...
            : 'Taku service search is unavailable.', controller.signal.aborted
            ? 'service_search_timeout'
            : 'service_search_unavailable', { error: error instanceof Error ? error.message : String(error) });
    }
    finally {
        clearTimeout(timer);
    }
    if (!response.ok) {
        throw new PublisherError(`Taku service search failed with HTTP ${response.status}.`, 'service_search_unavailable', { status: response.status });
    }
    const contentLength = Number(response.headers.get('content-length') || 0);
    if (Number.isFinite(contentLength) && contentLength > MAX_SERVICE_SEARCH_BYTES) {
        throw new PublisherError('Taku service search response is too large.', 'service_search_too_large');
    }
...
export async function discoverSubAppServiceCandidates(requirements, catalog, request) {
    const unresolved = requirements.filter((requirement) => requirement.mapping.status !== 'mapped');
    ...
    const discovered = await Promise.all(requirementsToSearch.map(async (requirement) => {
        ...
        try {
            ...
            const publisherError = error instanceof PublisherError
                ? error
                : new PublisherError('Taku service search is unavailable.', 'service_search_unavailable');
            return {
                requirementId: requirement.id,
                query,
                status: 'unavailable',
    ...
    const unavailable = discovered.filter((requirement) => requirement.status === 'unavailable').length;
    return {
...
function candidatesFromCatalog(results, catalog, limit) {
    const services = new Map(catalog.services.map((service) => [service.id, service]));
    const seen = new Set();
    const candidates = [];
    for (const result of results) {
        const service = services.get(result.serviceId);
        const endpoint = service?.endpoints.find((candidate) => candidate.id === result.endpointId);
...
function parseTakuServiceSearch(value, expectedQuery, limit) {
...
function searchQueryForRequirement(requirement) {
...
function normalizeSearchQuery(value) {
    const query = String(value || '')
        ...
        .slice(0, 300);
    if (!query) {
        throw new PublisherError('Taku service search query is empty.', 'service_search_query_invalid');
    }
    return query;
}
function normalizeSearchLimit(value) {
    ...
    if (!Number.isFinite(limit) || limit < 1 || limit > 10) {
        throw new PublisherError('Taku service search limit must be between 1 and 10.', 'service_search_limit_invalid');
    }
    return limit;
}
function normalizeSearchTimeout(value) {
    ...
    if (!Number.isFinite(timeoutMs) || timeoutMs < 1_000 || timeoutMs > 60_000) {
        throw new PublisherError('Taku service search timeout must be between 1 and 60 seconds.', 'service_search_timeout_invalid');
    }
    return timeoutMs;
}
...
function record(value, label) {
    if (!isRecord(value))
        throw invalidSearch(`${label} must be an object.`);
    return value;
}
function text(value, label) {
    const normalized = typeof value === 'string' ? value.trim() : '';
    if (!normalized)
        throw invalidSearch(`${label} is required.`);
    return normalized;
}
function optionalText(value) {
    return typeof value === 'string' ? value.trim() : '';
}
function invalidSearch(message) {
