...
import { checkSubAppConversion } from './subapp-agent.js';
...
export async function planSubAppRuntimeValidation(candidate, options = {}) {
    const checked = await (options.checkConversion || checkSubAppConversion)(candidate, conversionOptions(options));
    if (!checked.converted) {
        throw new PublisherError('SubApp candidate must pass the static conversion gate before runtime validation.', 'subapp_runtime_static_gate_required');
    }
    ...
    return {
        ...
        confirmationToken: runtimeConfirmationToken(checked.workspaceRoot, checked.candidateDigest, requirements),
        scriptsExecuted: false,
        publishStarted: false,
    };
}
export async function runSubAppRuntimeValidation(request, options = {}) {
    ...
    if (request.confirmationToken !== plan.confirmationToken) {
        throw new PublisherError('The trusted-runtime confirmation is missing, stale, or belongs to different candidate content.', 'subapp_runtime_confirmation_mismatch');
    }
    const requirements = {
        nodeVersion: plan.nodeVersion,
        pnpmVersion: plan.pnpmVersion,
    };
    const stateRoot = await secureDirectory(path.resolve(options.stateRoot ?? defaultRuntimeStateRoot()));
    const toolchain = await (options.ensureToolchain || (value => ensureManagedRuntimeToolchain(value, stateRoot)))(requirements);
    ...
    const evidenceRoot = await secureDirectory(path.join(evidenceParent, `runtime-${randomUUID()}`));
    ...
    const after = await (options.checkConversion || checkSubAppConversion)(request.candidate, conversionOptions(options));
    if (!after.converted || after.candidateDigest !== plan.candidateDigest) {
        throw new PublisherError('The original SubApp candidate changed during trusted runtime validation.', 'subapp_runtime_candidate_changed');
    }
    if (result.ok)
        await writeRuntimeReceipt(result);
    return result;
}
export async function ensureManagedRuntimeToolchain(requirements, stateRoot = defaultRuntimeStateRoot()) {
    if (process.platform !== 'darwin' || !['arm64', 'x64'].includes(process.arch)) {
        throw new PublisherError('Trusted SubApp runtime validation currently requires macOS Seatbelt.', 'subapp_runtime_platform_unsupported');
    }
    const nodeRelease = NODE_DISTRIBUTIONS[requirements.nodeVersion];
    const pnpmRelease = PNPM_DISTRIBUTIONS[requirements.pnpmVersion];
    if (!nodeRelease || !pnpmRelease) {
        throw new PublisherError('The candidate requests a runtime version that is not approved by this Publisher build.', 'subapp_runtime_version_unapproved', { node: requirements.nodeVersion, pnpm: requirements.pnpmVersion });
    }
    ...
    const nodeRoot = path.join(toolchainsRoot, `node-v${requirements.nodeVersion}-darwin-${architecture}`);
    ...
    if (!(await validNode(nodeExecutable, requirements.nodeVersion)) ||
        ...
        throw new PublisherError('The verified SubApp runtime toolchain could not be initialized.', 'subapp_runtime_toolchain_invalid');
    }
...
async function ensurePnpmExecutable(nodeRoot, pnpmCli) {
...
export async function runRepoToStaxRuntime(request) {
    const script = createRequire(import.meta.url).resolve('repo-to-stax-converter/runtime-cli');
    const result = await spawnCaptured(request.nodeExecutable, [
        script,
        'check',
        request.candidate,
        request.workRoot,
    ...
    ], request.candidate, 70 * 60_000);
    try {
        const output = JSON.parse(result.stdout);
        if (result.code !== 0 && !isRecord(output))
            throw new Error('missing failure report');
        return output;
    }
    catch {
        throw new PublisherError('Trusted SubApp runtime could not produce a validation report.', 'subapp_runtime_execution_failed', { exit_code: result.code, error: result.stderr.slice(0, 2_000) });
    }
}
function validateRuntimeEnvelope(value, plan, expectedEvidenceRoot) {
    ...
    const commandsPassed = commands.length === REQUIRED_COMMANDS.length &&
    ...
    const failurePhase = typeof envelope.failurePhase === 'string'
        ...
        : null;
    if (envelope.protocol !== RUNTIME_PROTOCOL ||
        ...
        throw new PublisherError('Trusted SubApp runtime returned an incompatible validation report.', 'subapp_runtime_contract_mismatch');
    }
    const ok = qualification.qualified === true &&
    ...
    if (typeof envelope.ok === 'boolean' && envelope.ok !== ok) {
        throw new PublisherError('Trusted SubApp runtime success status is inconsistent with its evidence.', 'subapp_runtime_contract_mismatch');
    }
    return {
        ok,
        ...
        scriptsExecutedInDisposableWorkspace: envelope.scriptsExecutedInDisposableWorkspace === true,
        buildArtifact,
        publishStarted: false,
        evidenceRoot: String(envelope.evidenceRoot ?? ''),
    };
}
async function writeRuntimeReceipt(result) {
    const receipt = {
    ...
    await fs.writeFile(path.join(result.evidenceRoot, 'runtime-receipt.json'), `${JSON.stringify(receipt, null, 2)}\n`, { encoding: 'utf8', mode: 0o600, flag: 'wx' });
}
function validBuildArtifact(value) {
...
async function readRuntimeRequirements(candidate) {
    ...
    if (!/^\d+\.\d+\.\d+\n$/.test(nvmrc)) {
        throw new PublisherError('SubApp candidate must pin one exact Node version in .nvmrc.', 'subapp_runtime_requirements_invalid');
    }
    let packageJson;
    try {
        ...
        throw new PublisherError('SubApp candidate package.json is invalid.', 'subapp_runtime_requirements_invalid');
    }
    const match = String(packageJson.packageManager ?? '').match(/^pnpm@(\d+\.\d+\.\d+)$/);
    if (!match?.[1]) {
        throw new PublisherError('SubApp candidate must pin one exact pnpm version.', 'subapp_runtime_requirements_invalid');
    }
    return { nodeVersion: nvmrc.trim(), pnpmVersion: match[1] };
}
function runtimeConfirmationToken(workspaceRoot, candidateDigest, requirements) {
...
async function installVerifiedArchive(options) {
    ...
    try {
        ...
        if (actual !== options.digest) {
            throw new PublisherError('Trusted runtime download failed its compiled checksum.', 'subapp_runtime_download_checksum_mismatch');
        }
        await fs.writeFile(archive, bytes, { mode: 0o600 });
        const extracted = await spawnCaptured('/usr/bin/tar', ['-xzf', archive, '--strip-components=1', '-C', stage], parent, 120_000);
        if (extracted.code !== 0) {
            throw new PublisherError('Trusted runtime archive could not be extracted.', 'subapp_runtime_archive_invalid');
        }
        try {
...
async function downloadPinnedArtifact(url) {
    ...
    if (parsed.protocol !== 'https:' ||
        ...
        throw new PublisherError('Trusted runtime artifact host is not approved.', 'subapp_runtime_download_host_unapproved');
    }
    let response;
    try {
        ...
        throw new PublisherError('Trusted runtime artifact could not be downloaded.', 'subapp_runtime_download_failed', { error: error instanceof Error ? error.message : String(error) });
    }
    if (!response.ok || !response.body) {
        throw new PublisherError('Trusted runtime artifact download returned an unexpected response.', 'subapp_runtime_download_failed', { status: response.status });
    }
    ...
    while (true) {
        ...
        if (length > MAX_TOOLCHAIN_DOWNLOAD_BYTES) {
            await reader.cancel();
            throw new PublisherError('Trusted runtime artifact exceeds the download limit.', 'subapp_runtime_download_too_large');
        }
        chunks.push(value);
    }
    return Buffer.concat(chunks.map(chunk => Buffer.from(chunk)), length);
}
async function validNode(executable, version) {
    try {
...
async function validPnpm(nodeExecutable, pnpmCli, version) {
    try {
        const metadata = await fs.lstat(pnpmCli);
        if (!metadata.isFile() || metadata.isSymbolicLink())
            return false;
        const result = await spawnCaptured(nodeExecutable, [pnpmCli, '--version'], path.dirname(pnpmCli), 10_000);
...
function normalizeCommandTimeout(value) {
    ...
    if (!Number.isInteger(timeout) || timeout < 1_000 || timeout > 120_000) {
        throw new PublisherError('Trusted runtime command timeout must be between 1 and 120 seconds.', 'subapp_runtime_timeout_invalid');
    }
    return timeout;
}
...
function requireRecord(value) {
    if (!isRecord(value)) {
        throw new PublisherError('Trusted SubApp runtime returned malformed evidence.', 'subapp_runtime_contract_mismatch');
    }
    return value;
}
