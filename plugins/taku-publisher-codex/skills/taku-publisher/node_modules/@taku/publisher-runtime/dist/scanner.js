...
import { atomicWriteJson, isRecord, nowIso, PublisherError, readJson, saveState, } from './util.js';
import { assertStageUnchanged } from './workspace.js';
const PLACEHOLDER_PATTERN = /(?:change[-_ ]?me|example|fake|placeholder|replace[-_ ]?me|sample|test|xxx+|your[-_ ]?(?:api[-_ ]?)?(?:key|token|secret|password)|<[^>]+>|\$\{[A-Z][A-Z0-9_]+\})/i;
...
const PRIVATE_URL_PATTERN = /https?:\/\/(?:localhost|127(?:\.\d{1,3}){3}|0\.0\.0\.0|10(?:\.\d{1,3}){3}|192\.168(?:\.\d{1,3}){2}|172\.(?:1[6-9]|2\d|3[01])(?:\.\d{1,3}){2}|\[[fF][eE]80:[^\]]+\]|[^/\s"'`]+\.(?:local|internal))(?:[:/?#\s"'`)]|$)/gi;
...
const CREDENTIAL_ASSIGNMENT_PATTERN = /^\s*(?:export\s+)?(?:(?:const|let|var)\s+)?([A-Za-z][A-Za-z0-9_.-]*)\s*[:=]\s*(.+)$/i;
...
export async function scanStaging(directory, state) {
    ...
    for (const entry of files) {
        ...
        if (isEnvTemplate(path.basename(file)))
            extractEnvTemplateRequirements(relative, text, requirements);
    }
    if (findings.length > 500) {
        findings = findings.slice(0, 500);
        findings.push(finding('finding_limit', 'block', '.', 0, 'The scan finding limit was exceeded; narrow or clean the package before publishing.', ''));
    }
...
export async function applyDeepScanDispositions(directory, state, sourceFile) {
    ...
    for (const value of dispositions.dispositions) {
        if (!isRecord(value))
            throw new PublisherError('Every disposition must be an object.', 'invalid_dispositions');
...
export async function assertScanReady(directory, state) {
    await assertStageUnchanged(directory, state);
    const report = requireRecord(await readJson(path.join(directory, 'scan-report.json')), 'invalid_scan_report');
    ...
    if (deep.blocked === true)
        throw new PublisherError('Deep scan contains a blocking disposition.', 'deep_scan_blocked');
}
export async function buildPlatformScanPayload(directory, state) {
    ...
    if (Array.isArray(request.generated_findings)) {
        for (const item of request.generated_findings.filter(isRecord))
            generated.set(String(item.id ?? ''), item);
    }
    const deepFindings = [];
    if (Array.isArray(deep.dispositions)) {
        for (const row of deep.dispositions.filter(isRecord)) {
            ...
            if (source)
                deepFindings.push(platformFinding(source, 'allow', 'warning', row.rationale));
        }
    }
...
function scanText(relative, text, findings) {
    for (const [index, line] of text.split(/\r?\n/).entries()) {
        ...
        if (!/\b(?:re\.compile|RegExp)\s*\(/.test(line)) {
            if (test(PRIVATE_KEY_PATTERN, line))
                findings.push(finding('private_key', 'block', relative, lineNumber, 'Private key material is not publishable.', line));
            if (test(KNOWN_TOKEN_PATTERN, line))
                findings.push(finding('known_token', 'block', relative, lineNumber, 'A value matches a known credential format.', line));
            const bearer = firstMatch(BEARER_PATTERN, line);
            if (bearer?.[1] && !PLACEHOLDER_PATTERN.test(bearer[1]))
                findings.push(finding('bearer_token', 'block', relative, lineNumber, 'A literal Bearer credential is not publishable.', line));
            const database = firstMatch(DATABASE_URL_PATTERN, line);
            if (database?.[1] && !PLACEHOLDER_PATTERN.test(database[1]))
                findings.push(finding('database_password_url', 'block', relative, lineNumber, 'A database URL contains embedded credentials.', line));
            if (test(NON_LOOPBACK_PRIVATE_URL_PATTERN, line))
                findings.push(finding('private_network_url', 'block', relative, lineNumber, 'A private-network URL is not portable or public.', line));
            if (test(LOOPBACK_URL_PATTERN, line))
                findings.push(finding('loopback_url', 'review', relative, lineNumber, 'A loopback URL requires portability review and must not appear in public metadata.', line));
            if (ABSOLUTE_PATH_PATTERNS.some((pattern) => test(pattern, line)))
                findings.push(finding('local_absolute_path', 'block', relative, lineNumber, 'A machine-specific absolute path is not publishable.', line));
        ...
        for (const [category, message, pattern] of RISK_PATTERNS) {
            if (pattern.test(line))
                findings.push(finding(category, 'review', relative, lineNumber, message, line));
        }
    }
}
function extractRequirements(relative, text, output) {
    for (const [index, line] of text.split(/\r?\n/).entries()) {
        for (const [pattern, required] of REQUIREMENT_PATTERNS) {
            pattern.lastIndex = 0;
            for (const match of line.matchAll(pattern)) {
                if (match[1])
                    recordRequirement(output, match[1], relative, index + 1, required);
            }
        }
    }
}
function extractEnvTemplateRequirements(relative, text, output) {
    for (const [index, line] of text.split(/\r?\n/).entries()) {
...
function recordRequirement(output, name, relative, line, required) {
...
function finding(category, severity, filePath, line, message, excerpt) {
...
function redactExcerpt(value) {
    ...
    for (const pattern of ABSOLUTE_PATH_PATTERNS)
        text = replace(pattern, text, ' [REDACTED LOCAL PATH]');
    const assignment = CREDENTIAL_ASSIGNMENT_PATTERN.exec(text);
    if (assignment?.[1] && assignment[2] && isSensitiveCredentialName(assignment[1]) && looksLikeLiteralSecret(assignment[2])) {
        const start = text.lastIndexOf(assignment[2]);
        if (start >= 0)
            text = `${text.slice(0, start)}[REDACTED]`;
    }
    return text;
}
function isSensitiveCredentialName(rawName) {
    return SECRET_NAME_PATTERN.test(rawName.split('.').at(-1) ?? rawName);
}
function looksLikeLiteralSecret(raw) {
    const value = raw.trim().replace(/[,;)]$/, '');
    if (PLACEHOLDER_PATTERN.test(value) || ENV_REFERENCE_PATTERN.test(value))
        return false;
    if (['none', 'null', 'undefined', 'true', 'false', '""', "''"].includes(value.toLowerCase()))
        return false;
    if (/^(?:await\s+)?[A-Za-z_$][A-Za-z0-9_$]*\s*\(/.test(value))
        return false;
    if (/^[{[(]|\.\.\./.test(value))
        return false;
    if (/^(?:new\s+|String\s*\(|Number\s*\(|Boolean\s*\()/.test(value))
        return false;
    ...
    if (candidate.startsWith('/'))
        return false;
    if (candidate.length < 10 || PLACEHOLDER_PATTERN.test(candidate))
        return false;
    if (test(KNOWN_TOKEN_PATTERN, candidate) || entropy(candidate) >= 3.2)
        return true;
...
async function normalizeAdditionalFindings(value, staging) {
    if (value === undefined || value === null)
        return [];
    if (!Array.isArray(value))
        throw new PublisherError('additional_findings must be an array.', 'invalid_dispositions');
    const output = [];
    for (const [index, raw] of value.entries()) {
        if (!isRecord(raw))
            throw new PublisherError('Every additional finding must be an object.', 'invalid_dispositions');
        ...
        if (!['allow', 'block', 'not_applicable'].includes(decision))
            throw new PublisherError('Additional finding decisions must be allow, block, or not_applicable.', 'invalid_dispositions');
        if (!safeStagedFile(staging, filePath))
            throw new PublisherError('Additional findings must reference a staged relative file.', 'invalid_dispositions');
        if (!Number.isInteger(line) || line < 0)
            throw new PublisherError('Additional finding line must be a non-negative integer.', 'invalid_dispositions');
        if (message.length < 5 || rationale.length < 10)
            throw new PublisherError('Additional findings need a message and rationale.', 'invalid_dispositions');
...
async function applyRequirementUpdates(directory, value, staging) {
    if (value === undefined || value === null)
        return [];
    if (!Array.isArray(value))
        throw new PublisherError('requirement_updates must be an array.', 'invalid_dispositions');
    const requirements = requireRecord(await readJson(path.join(directory, 'requirements.json')), 'invalid_requirements');
    const existing = new Map();
    for (const bucket of ['secrets', 'env']) {
        const rows = requirements[bucket];
        if (Array.isArray(rows))
            for (const item of rows.filter(isRecord))
    ...
    for (const raw of value) {
        if (!isRecord(raw))
            throw new PublisherError('Every requirement update must be an object.', 'invalid_dispositions');
        if ('value' in raw || 'default' in raw)
            throw new PublisherError('Requirement updates cannot contain values or defaults.', 'secret_in_dispositions');
        const name = String(raw.name ?? '').trim();
        const kind = String(raw.kind ?? '').trim();
        const required = raw.required;
        const purpose = cleanRationale(raw.purpose);
        if (!/^[A-Z][A-Z0-9_]{1,127}$/.test(name) || seen.has(name))
            throw new PublisherError('Requirement names must be unique uppercase environment names.', 'invalid_dispositions');
        if (!['secret', 'env'].includes(kind) || typeof required !== 'boolean' || purpose.length < 8)
            throw new PublisherError('Requirement updates need kind, required, and a meaningful purpose.', 'invalid_dispositions');
        ...
        if (!sources.length)
            throw new PublisherError('A new semantic requirement needs relative source evidence.', 'invalid_dispositions');
        assertNoSecretValue(purpose);
        const item = { name, kind, required, purpose, sources };
...
function normalizeRequirementSources(value, staging) {
    if (value === undefined || value === null)
        return [];
    if (!Array.isArray(value))
        throw new PublisherError('Requirement sources must be an array.', 'invalid_dispositions');
    const output = [];
    for (const raw of value) {
        if (!isRecord(raw))
            throw new PublisherError('Requirement source evidence must be an object.', 'invalid_dispositions');
        const filePath = String(raw.path ?? '').trim();
        const line = Number(raw.line ?? 0);
        if (!safeStagedFile(staging, filePath))
            throw new PublisherError('Requirement evidence must reference a staged relative file.', 'invalid_dispositions');
        if (!Number.isInteger(line) || line < 0)
            throw new PublisherError('Requirement source line must be a non-negative integer.', 'invalid_dispositions');
        if (!output.some((item) => item.path === filePath && item.line === line))
            output.push({ path: filePath, line });
    }
    return output;
}
function platformRequirements(value, allowDefault) {
    if (!Array.isArray(value))
        return [];
    return value.filter(isRecord).map((item) => {
        ...
        if (allowDefault && typeof item.default === 'string' && item.default.trim())
            normalized.default = item.default.trim();
        return normalized;
    });
}
function platformFinding(source, disposition, severity, rationale) {
    let message = String(source.message ?? 'Security review finding.').trim();
    const rationaleText = String(rationale ?? '').trim();
    if (rationaleText)
        message = `${message} Review: ${rationaleText}`;
    const result = {
    ...
    if (filePath)
        result.path = filePath.slice(0, 1024);
    if (Number.isInteger(source.line) && Number(source.line) >= 1)
        result.line = Number(source.line);
    return result;
}
function assertNoSecretValue(text) {
...
async function readTextFile(file, size) {
...
function isEnvTemplate(name) {
    return /^\.env(?:[._-](?:example|sample|template))$/i.test(name);
}
function safeStagedFile(staging, relative) {
...
function requireRecord(value, code) {
    if (!isRecord(value))
        throw new PublisherError('Expected a JSON object.', code);
    return value;
}
