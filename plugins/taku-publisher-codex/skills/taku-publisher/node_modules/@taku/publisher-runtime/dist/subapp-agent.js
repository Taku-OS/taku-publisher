...
import { assertSubAppValidationResult, } from '@taku/subapp-contract';
import { runRepoToStaxAgentCommand, } from './subapp-assessment.js';
...
export async function createSubAppAgentHandoff(candidate, options = {}) {
    const workspaceRoot = await normalizeCandidate(candidate);
    const output = await runAgentConverter('handoff', workspaceRoot, options);
    const envelope = validateEnvelope(output, AGENT_HANDOFF_PROTOCOL, workspaceRoot);
    const validation = validateResult(envelope.validation, 'workspace');
    if (!validation.ok) {
        throw new PublisherError('SubApp candidate is not safe to hand to the conversion Agent.', 'subapp_agent_handoff_rejected', { errors: validation.errors });
    }
    const agentContract = requireRecord(envelope.agentContract, 'SubApp Agent handoff is missing its contract.');
    ...
    if (JSON.stringify(normalizedContract.requiredReads) !== JSON.stringify(REQUIRED_READS) ||
        ...
        throw new PublisherError('SubApp Agent handoff contract is incompatible with this Publisher build.', 'subapp_agent_contract_mismatch');
    }
    return {
        workspaceRoot,
        ...
        validation,
        agentContract: normalizedContract,
    };
}
export async function checkSubAppConversion(candidate, options = {}) {
    const workspaceRoot = await normalizeCandidate(candidate);
    const output = await runAgentConverter('validate-conversion', workspaceRoot, options);
    const envelope = validateEnvelope(output, CONVERSION_CHECK_PROTOCOL, workspaceRoot);
    const validation = validateResult(envelope.validation, 'conversion');
    if (envelope.scriptsExecuted !== false) {
        throw new PublisherError('SubApp conversion check crossed the static-validation boundary.', 'subapp_agent_contract_mismatch');
    }
...
async function runAgentConverter(command, candidate, options) {
...
async function normalizeCandidate(candidate) {
    ...
    if (!path.isAbsolute(requested)) {
        throw new PublisherError('SubApp candidate must be an absolute directory path.', 'subapp_candidate_not_absolute');
    }
    try {
        const canonical = await fs.realpath(requested);
        const metadata = await fs.lstat(canonical);
        if (canonical !== path.resolve(requested) ||
        ...
        throw new PublisherError('SubApp candidate does not exist or has an unsafe path.', 'subapp_candidate_unreadable');
    }
}
function validateEnvelope(output, protocol, workspaceRoot) {
    const envelope = requireRecord(output, 'SubApp Converter Agent output must be an object.');
    if (stringValue(envelope.protocol) !== protocol ||
        !SUPPORTED_CONVERTER_VERSIONS.has(stringValue(envelope.converterVersion)) ||
        stringValue(envelope.workspaceRoot) !== workspaceRoot) {
        throw new PublisherError('SubApp Converter Agent protocol is incompatible with this Publisher build.', 'subapp_agent_converter_incompatible');
    }
    return envelope;
}
function validateResult(value, level) {
    try {
        const validation = assertSubAppValidationResult(value);
        if (validation.level !== level)
            throw new Error(`Expected ${level} validation.`);
        return validation;
    }
    catch (error) {
        throw new PublisherError('SubApp Converter returned an invalid validation result.', 'subapp_agent_contract_mismatch', { error: error instanceof Error ? error.message : String(error) });
    }
}
function migrationSummary(value) {
    const migration = requireRecord(value, 'SubApp Agent output is missing migration provenance.');
    const status = stringValue(migration.status);
    if (stringValue(migration.schemaVersion) !== 'taku.subapp-migration.v2' ||
        ...
        throw new PublisherError('SubApp Agent migration provenance is invalid.', 'subapp_agent_contract_mismatch');
    }
    return migration;
}
function digestValue(value) {
    ...
    if (!/^sha256:[a-f0-9]{64}$/.test(digest)) {
        throw new PublisherError('SubApp Agent candidate digest is invalid.', 'subapp_agent_contract_mismatch');
    }
    return digest;
}
