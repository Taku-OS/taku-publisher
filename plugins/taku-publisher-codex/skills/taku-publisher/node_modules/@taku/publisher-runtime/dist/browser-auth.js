...
export async function loginWithBrowser(options) {
    ...
    if (!options.onEvent)
        process.stderr.write('Waiting for browser confirmation. Open the authorization URL if no page appears. Keep this command running; authorization will resume it automatically.\n');
    let browserLaunch = { status: 'skipped' };
    let received;
    let timeoutId;
    let onAbort;
    try {
        ...
        received = await Promise.race([
            ...
            new Promise((_, reject) => {
                timeoutId = setTimeout(() => reject(new PublisherError('Authorization timed out; this login link is no longer valid. Run the original command again to obtain a new link.', 'auth_timeout', { browser_launch: { ...browserLaunch } })), Math.max(1, deadline - Date.now()));
            }),
        ]);
    }
    finally {
