...
export const REPO_TO_STAX_AGENT_HANDOFF_PROTOCOL = 'repo-to-stax.agent-handoff.v1';
export const REPO_TO_STAX_CONVERSION_CHECK_PROTOCOL = 'repo-to-stax.conversion-check.v1';
...
export async function createAgentHandoff(candidate) {
    ...
    if (!validation.ok) {
        throw new Error(`Candidate failed workspace validation: ${validation.errors.join('; ')}`);
    }
...
export async function checkAgentConversion(candidate) {
    const workspaceRoot = await normalizeCandidate(candidate);
    const candidateDigest = await computeTreeDigest(workspaceRoot);
    const validation = await validateSubAppWorkspace(workspaceRoot, {
        level: 'conversion',
...
async function readMigrationSummary(workspaceRoot) {
    const parsed = JSON.parse(await readFile(join(workspaceRoot, '.taku', 'migration.json'), 'utf8'));
    ...
    if (summary.schemaVersion !== 'taku.subapp-migration.v2' ||
...
async function main() {
    ...
    if (rest.length > 0 || !candidate) {
        throw new Error('Usage: agent-cli <handoff|validate-conversion> <absolute-candidate-path>');
    }
