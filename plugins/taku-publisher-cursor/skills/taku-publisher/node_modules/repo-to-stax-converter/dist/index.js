...
import { assertFreshConverterBuild, converterProjectRootForCompiledRuntime, } from './lib/build-freshness.js';
...
const runtimeProjectRoot = await converterProjectRootForCompiledRuntime(fileURLToPath(import.meta.url));
if (runtimeProjectRoot)
    await assertFreshConverterBuild(runtimeProjectRoot);
...
program
    ...
    .requiredOption('--template <repo-or-path>', 'Explicit local Taku template root or remote GitHub template')
...
program
    ...
    .option('--trusted-attestation <path>', 'Untrusted publish evidence; authority is unavailable in this build')
    .description('Validate a generated Taku Stax SubApp workspace')
    .action(async (subappPath, options) => {
    ...
    if (options.trustedAttestation) {
        attestationEvidence = JSON.parse(await readFile(resolve(options.trustedAttestation), 'utf8'));
    }
...
program
    ...
    .action(async (subappPath) => {
    const result = await checkAgentConversion(resolve(subappPath));
    console.log(JSON.stringify(result, null, 2));
