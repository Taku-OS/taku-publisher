...
export async function scanSecretLikeFiles(root, options = {}) {
    const found = new Set();
    ...
    async function walk(relativeRoot) {
        ...
        for (const entry of entries) {
            ...
            if (!entry.isFile())
                continue;
            if (isSecretLikeFilename(entry.name))
                found.add(relativePath);
            if (isKnownBinaryFilename(entry.name))
                continue;
...
function isSecretLikeFilename(name) {
    ...
    if (lower === '.env.example' || lower === '.example.env')
        return false;
    return (lower === '.env' ||
...
function isKnownBinaryFilename(name) {
    const lower = name.toLowerCase();
    return /\.(?:png|jpe?g|gif|webp|avif|ico|pdf|zip|gz|tgz|bz2|xz|7z|rar|woff2?|ttf|otf|mp[34]|mov|avi|webm|sqlite3?|db|wasm|node)$/.test(lower);
}
...
const SECRET_NAME_PATTERN = new RegExp(`(?:^|_)(?:${SECRET_NAME_MARKERS.join('|')})(?:_|$)`);
...
export function containsSecretLikeText(content, options = {}) {
    if (KNOWN_CREDENTIAL_PATTERNS.some(pattern => pattern.test(content)))
        return true;
    if (options.filePath && isJavaScriptLikePath(options.filePath)) {
        ...
        try {
            ...
            const sourceFile = createSourceFile(options.filePath, content, ts.ScriptTarget.Latest, true, getScriptKind(options.filePath));
            const analysis = analyzeSecretLikeAst(sourceFile);
            if (analysis.found)
                return true;
            if (containsSecretLikeComment(sourceFile))
                return true;
            const parseDiagnostics = sourceFile.parseDiagnostics;
            if (Array.isArray(parseDiagnostics) && parseDiagnostics.length === 0)
                return false;
            return containsSecretLikeTextFallback(content, getFallbackExcludedNameRanges(sourceFile, analysis.safeNameRanges));
        }
        catch {
            return containsSecretNameToken(content);
        }
...
function containsSecretLikeComment(sourceFile) {
    const content = sourceFile.text;
    const ranges = new Map();
    const literalTextRanges = [];
    ...
    for (const { start, end } of commentRanges) {
        while (literalIndex < mergedLiteralTextRanges.length &&
            mergedLiteralTextRanges[literalIndex].end <= start) {
            literalIndex += 1;
        }
        const literalRange = mergedLiteralTextRanges[literalIndex];
        if (literalRange && literalRange.start < end && literalRange.end > start)
            continue;
        if (containsSecretLikeTextFallback(content.slice(start, end), new Map(), {
...
function getScriptKind(filePath) {
    ...
    if (/\.(?:mjs|cjs|js)$/.test(lower))
        return ts.ScriptKind.JS;
    return ts.ScriptKind.TS;
}
function analyzeSecretLikeAst(sourceFile) {
    let found = false;
    const safeNameRanges = [];
    function inspect(target, value, operator = ts.SyntaxKind.EqualsToken) {
        if (!target || !isSecretName(target.name))
            return false;
        const storageTarget = getSecretStorageTarget(target.name);
        if (storageTarget) {
            const safe = operator === ts.SyntaxKind.EqualsToken &&
                isSafeSecretStorageExpression(storageTarget, value);
            if (safe)
                safeNameRanges.push(target);
            return !safe;
        }
        if (isExactSafeSecretExpression(value))
            return false;
        return (containsPlausibleSecretLiteral(value) ||
    ...
    function visit(node) {
        ...
        if (ts.isVariableDeclaration(node) && node.initializer) {
            found = collectBindingTargets(node.name, sourceFile).some(target => inspect(target, node.initializer));
        }
        else if (ts.isParameter(node) && node.initializer) {
            found = collectBindingTargets(node.name, sourceFile).some(target => inspect(target, node.initializer));
        }
        else if (ts.isBindingElement(node) && node.initializer) {
            found = collectBindingElementTargets(node, sourceFile).some(target => inspect(target, node.initializer));
        }
            ...
            found = inspect(getStaticTarget(node.name, sourceFile), node.objectAssignmentInitializer);
        }
            ...
            found = collectReturnedExpressions(node.body).some(expression => inspect(target, expression));
        }
        else if (ts.isSetAccessorDeclaration(node)) {
            const target = getStaticTarget(node.name, sourceFile);
            found = node.parameters.some(parameter => collectDefaultExpressions(parameter).some(expression => inspect(target, expression)));
        }
        else if (ts.isBinaryExpression(node) && isAssignmentOperator(node.operatorToken.kind)) {
            found = collectAssignmentTargets(node.left, sourceFile).some(target => inspect(target, node.right, node.operatorToken.kind));
        }
...
function getStaticTarget(name, sourceFile) {
...
function collectBindingTargets(name, sourceFile) {
    if (ts.isIdentifier(name)) {
...
function collectBindingElementTargets(element, sourceFile) {
...
function collectAssignmentTargets(expression, sourceFile) {
...
function collectForInitializerTargets(initializer, sourceFile) {
    if (ts.isVariableDeclarationList(initializer)) {
        return initializer.declarations.flatMap(declaration => collectBindingTargets(declaration.name, sourceFile));
    }
    return collectAssignmentTargets(initializer, sourceFile);
}
function getStaticJsxAttributeTarget(name, sourceFile) {
...
function collectDefaultExpressions(parameter) {
...
function isSafeSecretStorageExpression(target, expression) {
    if (isNonOptionalStaticPropertyAccess(expression))
        return true;
    if (!ts.isStringLiteral(expression) && !ts.isNoSubstitutionTemplateLiteral(expression)) {
        return false;
    }
...
function isNonOptionalStaticPropertyAccess(expression) {
    if (!ts.isPropertyAccessExpression(expression))
        return false;
...
function isExactSafeSecretExpression(expression) {
    if (isExactEnvironmentReference(expression))
        return true;
    if (ts.isStringLiteral(expression) || ts.isNoSubstitutionTemplateLiteral(expression)) {
...
function isExactEnvironmentReference(expression) {
    if (!ts.isPropertyAccessExpression(expression) || expression.questionDotToken)
        return false;
    const environment = expression.expression;
    if (!ts.isPropertyAccessExpression(environment) || environment.questionDotToken)
        return false;
    if (environment.name.text !== 'env')
        return false;
    if (ts.isIdentifier(environment.expression))
        return environment.expression.text === 'process';
    return (ts.isMetaProperty(environment.expression) &&
        environment.expression.keywordToken === ts.SyntaxKind.ImportKeyword &&
        environment.expression.name.text === 'meta');
}
function containsPlausibleSecretLiteral(expression) {
    let found = false;
    function visit(node) {
        if (found)
            return;
...
function isAmbiguousMultilineSecretExpression(expression, sourceFile) {
    const isAmbiguousOperator = ts.isAsExpression(expression) ||
    ...
    if (!isAmbiguousOperator)
        return false;
    const text = sourceFile.text.slice(expression.getStart(sourceFile), expression.end);
    return text.includes('\n') || text.includes('\r');
}
function hasAmbiguousTrailingContinuation(expression, sourceFile) {
    const content = sourceFile.text;
...
function getFallbackExcludedNameRanges(sourceFile, safeNameRanges) {
    const excluded = new Map();
    function exclude(target) {
        if (!target)
            return;
        const ends = excluded.get(target.start) ?? new Set();
        ends.add(target.end);
        excluded.set(target.start, ends);
    }
    function excludeBindingName(name) {
        for (const target of collectBindingTargets(name, sourceFile))
            exclude(target);
    }
    for (const target of safeNameRanges)
        exclude(target);
    function visit(node) {
        if (ts.isParameter(node) && !node.initializer) {
            ...
            for (const target of collectBindingElementTargets(node, sourceFile))
                exclude(target);
        }
        ts.forEachChild(node, visit);
    }
...
function containsSecretNameToken(content) {
    let index = 0;
    while (index < content.length) {
        ...
        let nameEnd = index + 1;
        while (isUppercaseIdentifierCode(content.charCodeAt(nameEnd)))
            nameEnd += 1;
        if (!isIdentifierCode(content.charCodeAt(nameEnd))) {
            if (isSecretName(content.slice(index, nameEnd)))
                return true;
        }
        index = nameEnd;
    }
...
function containsSecretLikeTextFallback(content, excludedNameRanges = new Map(), options = {}) {
    ...
    while (index < content.length) {
        ...
        while (pythonEnvironmentTargetIndex < pythonEnvironmentTargetRanges.length &&
            pythonEnvironmentTargetRanges[pythonEnvironmentTargetIndex].end <= index) {
            pythonEnvironmentTargetIndex += 1;
        }
        const pythonEnvironmentTarget = pythonEnvironmentTargetRanges[pythonEnvironmentTargetIndex];
        if (pythonEnvironmentTarget &&
            pythonEnvironmentTarget.start <= index &&
            index < pythonEnvironmentTarget.end) {
            const safeStatementEnd = getSafePythonEnvironmentStatementEnd(content, index, nameEnd, pythonEnvironmentTarget);
            if (safeStatementEnd === null)
                return true;
            index = safeStatementEnd;
            continue;
        }
        while (pythonEnvironmentReadIndex < pythonEnvironmentReadRanges.length &&
        ...
        if (assignment.commentFinding)
            return true;
        if (!assignment.found) {
            index = Math.max(nameEnd, assignment.resumeAt);
            continue;
        }
        const value = readFallbackAssignmentValue(content, assignment.valueStart, commentsAreTrivia, options.markdownPhysicalLinesAreBoundaries ?? false);
...
function analyzePythonEnvironmentOccurrences(content) {
    ...
    for (const occurrence of occurrences) {
        ...
        const statementStart = occurrence.isCode && /^[ \t]*$/.test(content.slice(occurrenceLine.start, occurrence.start));
        let cursor = skipBroadPythonTargetTrivia(content, occurrence.start + 'os.environ'.length, occurrenceEnd);
        if (content[cursor] !== '[')
            continue;
        ...
        coveredUntil = cursor;
        const assignmentIntent = occurrence.isCode &&
            (statementStart
...
function indexPhysicalLines(content) {
    const lines = [];
    let start = 0;
    let cursor = 0;
    while (cursor < content.length) {
        ...
        const newline = character === '\r' && content[cursor + 1] === '\n'
            ? '\r\n'
            : character;
...
function findPythonEnvironmentOccurrences(content, lines) {
    const occurrences = [];
    let quote = null;
    let lineIndex = 0;
    let cursor = 0;
    let continuedSingleQuote = false;
    let stringOccurrences = [];
    const closeString = (end) => {
        for (const occurrence of stringOccurrences)
            occurrence.nonCodeEnd = end;
        stringOccurrences = [];
    };
    ...
    if (quote)
        closeString(content.length);
    return occurrences;
}
function skipBroadPythonTargetTrivia(content, start, end) {
    let cursor = start;
    while (cursor < end) {
...
function hasPythonStatementAssignmentIntent(content, lines, startLineIndex, start) {
    ...
    for (let lineIndex = startLineIndex; lineIndex < lines.length; lineIndex += 1) {
        ...
        if (extensionDepth === 0) {
            if (isPythonComparisonAt(content, cursor))
                return false;
            if (isStandalonePythonAssignmentAt(content, cursor))
                return true;
            if (!isPythonTargetExtensionStart(content[cursor] ?? ''))
                return false;
        }
        let quote = null;
        while (cursor < line.end) {
...
function hasImmediatePythonAssignmentIntent(content, lineEnd, start) {
    let cursor = start;
...
function isStandalonePythonAssignmentAt(content, index) {
    if (content[index] !== '=')
        return false;
    return content[index + 1] !== '=' && !'=!<>'.includes(content[index - 1] ?? '');
}
function isPythonComparisonAt(content, index) {
    return (content.startsWith('==', index) ||
        content.startsWith('!=', index) ||
        content.startsWith('<=', index) ||
        content.startsWith('>=', index));
}
function isPythonTargetExtensionStart(character) {
    return character !== '' && `.,;()[]{}'"\\!?:+-*/%@&|^<>`.includes(character);
}
function isHorizontalPythonWhitespace(character) {
    return (character !== '' &&
        /^\s$/u.test(character) &&
        character !== '\r' &&
        character !== '\n');
}
function isPythonIdentifierCharacter(character) {
...
function isSimplePythonSymbolicReference(value) {
    if (!/^[A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)*$/.test(value)) {
        return false;
    }
    return value.split('.').every(segment => !PYTHON_NON_REFERENCE_WORD.test(segment));
}
function getSafePythonEnvironmentStatementEnd(content, nameStart, nameEnd, target) {
    const { line } = target;
    if (line.newline === '\r')
        return null;
    const text = content.slice(line.start, line.end);
    const match = /^([ \t]*os\.environ[ \t]*\[[ \t]*(["']))([A-Z][A-Z0-9_]*)(\2[ \t]*\][ \t]*=[ \t]*)([A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)*)([ \t]*)(#.*)?$/.exec(text);
    if (!match ||
...
function isBroadWhitespace(character) {
    return character !== '' && /^\s$/u.test(character);
}
function findFallbackAssignment(content, nameStart, nameEnd, commentsAreTrivia) {
    let cursor = nameEnd;
...
function skipFallbackTrivia(content, start, commentsAreTrivia) {
    let cursor = start;
    while (cursor < content.length) {
        const character = content[cursor];
        ...
        if (commentsAreTrivia && content.startsWith('/*', cursor)) {
            ...
            if (containsSecretLikeTextFallback(content.slice(cursor, commentEnd), new Map(), {
                commentsAreTrivia: false,
            })) {
                return { end: commentEnd, commentFinding: true };
            }
            if (end < 0)
                return { end: content.length, commentFinding: false };
            cursor = end + 2;
            continue;
        }
        if (commentsAreTrivia && content.startsWith('//', cursor)) {
...
function readFallbackAssignmentValue(content, start, commentsAreTrivia, markdownPhysicalLinesAreBoundaries) {
    let valueStart = start;
    while (content[valueStart] === ' ' || content[valueStart] === '\t')
        valueStart += 1;
    if (content[valueStart] === '\r' || content[valueStart] === '\n') {
        let next = valueStart;
        while (isWhitespace(content[next] ?? ''))
            next += 1;
    ...
    while (cursor < content.length) {
        ...
        if (character === '\r' || character === '\n') {
            ...
            if (markdownPhysicalLinesAreBoundaries) {
                ...
                if (character === '\r' && content[nextLine] === '\n')
                    nextLine += 1;
                while (content[nextLine] === ' ' || content[nextLine] === '\t')
                    nextLine += 1;
                const dangerousContinuation = content[nextLine] !== '`' && isFallbackExpressionContinuation(content, nextLine);
                return {
                    value: content.slice(valueStart, cursor).trim(),
        ...
        else if (!markdownPhysicalLinesAreBoundaries &&
...
function isFallbackExpressionContinuation(content, start) {
    const character = content[start] ?? '';
    if (character && '()[].?+-*/%&|^<>=!`'.includes(character))
        return true;
    return ['in', 'instanceof', 'as', 'satisfies'].some(keyword => startsWithFallbackWord(content, start, keyword));
}
function startsWithFallbackWord(content, start, word) {
    if (!content.startsWith(word, start))
        return false;
    return !isIdentifierCode(content.charCodeAt(start + word.length));
}
function isSecretName(name) {
    if (!name || !isUppercaseAscii(name.charCodeAt(0)))
        return false;
    for (let index = 1; index < name.length; index += 1) {
        if (!isUppercaseIdentifierCode(name.charCodeAt(index)))
            return false;
    }
    return SECRET_NAME_PATTERN.test(name);
}
function isUppercaseAscii(code) {
    return code >= 65 && code <= 90;
}
function isUppercaseIdentifierCode(code) {
    return isUppercaseAscii(code) || (code >= 48 && code <= 57) || code === 95;
}
function isIdentifierCode(code) {
    return (isUppercaseIdentifierCode(code) ||
        (code >= 97 && code <= 122) ||
        code === 36);
}
...
function getSecretStorageTarget(name) {
    for (const marker of SECRET_NAME_MARKERS) {
        const match = name.match(new RegExp(`(?:^|_)${marker}_STORAGE_(KEY|PREFIX)$`));
        if (match)
            return { kind: match[1], marker };
    }
    return null;
}
function isSafeSecretStorageValue(target, value) {
    if (/^[A-Za-z_$][A-Za-z0-9_$]*(?:\.[A-Za-z_$][A-Za-z0-9_$]*)+$/.test(value)) {
        return true;
    }
...
function isSafeSecretStorageLabel(target, label) {
    const trailingDelimiter = target.kind === 'PREFIX' ? ':?' : '';
    const markerWords = target.marker.toLowerCase().split('_').join('[:_-]');
    return new RegExp(`^[a-z][a-z0-9]*(?:[:_-][a-z][a-z0-9]*)*[:_-]${markerWords}${trailingDelimiter}$`).test(label);
}
...
function isPlausibleSecretLiteral(value) {
...
function isControlledPlaceholder(value) {
    return /^(?:(?:placeholder|example|dummy|fake|redacted|change-me|changeme)(?:[-_:](?:value|token|secret|key|api[-_]?key|credential))?|your[-_](?:value|token|secret|key|api[-_]?key|credential)(?:[-_]here)?)$/.test(value.toLowerCase());
}
