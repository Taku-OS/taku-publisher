...
export async function writeConverterBuildManifest(projectRoot) {
    const sourceDigest = await computeTreeDigest(join(projectRoot, 'src'));
    const distRoot = join(projectRoot, 'dist');
    await mkdir(distRoot, { recursive: true });
    await writeFile(join(distRoot, BUILD_MANIFEST_NAME), `${JSON.stringify({ schemaVersion: BUILD_MANIFEST_SCHEMA, sourceDigest }, null, 2)}\n`);
}
export async function assertFreshConverterBuild(projectRoot) {
    let parsed;
    try {
        parsed = JSON.parse(await readFile(join(projectRoot, 'dist', BUILD_MANIFEST_NAME), 'utf8'));
    }
    catch {
    ...
    try {
        ...
        throw new Error('Converter source tree is unavailable; reinstall the package including src or run pnpm build.');
    }
    if (sourceDigest !== parsed.sourceDigest) {
        throw new Error('Converter dist is stale relative to src; run pnpm build.');
    }
}
export async function converterProjectRootForCompiledRuntime(runtimeEntryFile) {
    const canonicalEntry = await realpath(runtimeEntryFile);
    if (extname(canonicalEntry) !== '.js')
        return null;
