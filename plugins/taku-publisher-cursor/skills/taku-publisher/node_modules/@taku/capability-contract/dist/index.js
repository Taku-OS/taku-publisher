...
export const CAPABILITY_CONTRACT_VERSION = '0.2.0';
export const CAPABILITY_SNAPSHOT_SCHEMA_VERSION = 'taku.capability-snapshot.v1';
export const CAPABILITY_PACKAGE_SCHEMA_VERSION = 'taku.package.v1';
export const LEGACY_AI_SETUP_SCHEMA_VERSION = 'taku.ai-setup.v1';
...
const SECRET_QUERY_PATTERN = /[?&#](?:access[_-]?token|refresh[_-]?token|id[_-]?token|api[_-]?key|client[_-]?secret|password|passwd|secret|session|token)=/i;
const SHA256_PATTERN = /^[a-f0-9]{64}$/i;
const RELATIVE_PACKAGE_PATH_PATTERN = /^(?!\/)(?![A-Za-z]:[\\/])(?!.*(?:^|\/)\.\.(?:\/|$))[A-Za-z0-9._@+ -]+(?:\/[A-Za-z0-9._@+ -]+)*$/;
export function normalizeCapabilityKind(value) {
    return KIND_ALIASES.get(String(value || '').trim().toLowerCase()) || null;
}
export function normalizeCapabilitySource(value) {
...
export function defaultCapabilityPolicy(kind, options = {}) {
...
export function stableCapabilityId(input) {
    const kind = normalizeCapabilityKind(input.kind);
    if (!kind)
        throw new TypeError('Stable capability ID requires a valid kind.');
    const source = normalizeCapabilitySource(input.source);
    const canonicalLocator = cleanLocator(input.locator || input.detectedFrom || input.name);
    if (!canonicalLocator) {
        throw new TypeError('Stable capability ID requires a locator, detected source, or name.');
    }
...
export function normalizeCapabilityItem(item, privateItem = {}) {
    ...
    const localPath = cleanText(privateItem?.localPath || item?.localPath, 4096);
    const detectedFrom = cleanText(item?.detectedFrom, 512);
    const id = cleanText(item?.id, 512) ||
        stableCapabilityId({
            kind,
            source,
            name,
    ...
    return {
        ...
        kind,
        source,
        sourceLabel: cleanText(item?.sourceLabel, 120) || capabilitySourceLabel(source),
        sourceFormat: cleanText(item?.sourceFormat, 120) ||
            inferSourceFormat(kind, source, localPath),
        name,
        description: cleanText(item?.description, 2000),
        ...(detectedFrom ? { detectedFrom } : {}),
        ...(localPath
            ? { locator: { type: 'local-path', value: localPath } }
            : {}),
...
export function createCapabilitySnapshot(input = {}) {
    const privateById = new Map((input.privateItems || [])
        .filter((item) => item?.id)
        .map((item) => [String(item.id), item]));
    const items = (input.items || [])
        .map((item) => normalizeCapabilityItem(item, privateById.get(String(item?.id))))
        .filter((item) => Boolean(item))
        .sort(compareCapabilityItems);
    const generatedAt = validTimestamp(input.generatedAt) || new Date().toISOString();
    const scanId = cleanText(input.scanId, 512) ||
        createHash('sha256')
            .update(`${generatedAt}\0${items
            .map((item) => item.id)
            .sort()
            .join('\0')}`)
            .digest('hex')
...
export function assertCapabilitySnapshot(value) {
    ...
    if (value.schemaVersion !== CAPABILITY_SNAPSHOT_SCHEMA_VERSION) {
        throw new TypeError(`Unsupported capability snapshot schema: ${String(value.schemaVersion || 'missing')}.`);
    }
    if (value.contractVersion !== CAPABILITY_CONTRACT_VERSION) {
        throw new TypeError(`Unsupported capability contract version: ${String(value.contractVersion || 'missing')}.`);
    }
    ...
    for (const item of value.items) {
        ...
        if (!isRecord(item.policy)) {
            throw new TypeError(`Capability item "${String(item.id)}" is missing policy.`);
        }
        ...
        if (!isRecord(importPolicy) || !isRecord(publishPolicy)) {
            throw new TypeError(`Capability item "${String(item.id)}" is missing import/publish policy.`);
        }
    }
    return value;
}
export function readCapabilitySnapshot(value) {
    if (isRecord(value) &&
...
export function projectCapabilityKindForPackage(kind, channel) {
    return channel === 'publish' && kind === 'workflow' ? 'action' : kind;
}
export function createCapabilityPackageManifest(input = {}) {
    const channel = input.channel === 'publish'
        ? 'publish'
        : input.channel === 'import'
            ? 'import'
            : null;
    const sourceKind = normalizeCapabilityKind(input.kind || input.capability?.kind);
    if (!channel || !sourceKind) {
        throw new TypeError('Capability package requires a valid channel and kind.');
    }
    if (channel === 'publish' && (sourceKind === 'mcp' || sourceKind === 'rule')) {
        throw new TypeError(`${sourceKind} capabilities are not publishable in taku.package.v1.`);
    }
    const contentHash = cleanText(input.contentHash, 256);
    if (!SHA256_PATTERN.test(contentHash)) {
        throw new TypeError('Capability package requires a SHA-256 contentHash.');
    }
    ...
    const requiredSecrets = uniqueStrings(input.requiredSecrets).filter((name) => /^[A-Z][A-Z0-9_]{1,127}$/.test(name));
    const permissionReview = normalizePluginPermissionReview(input.permissionReview);
    if (sourceKind === 'plugin' && channel === 'publish') {
        if (!permissionReview) {
            throw new TypeError('Plugin publishing requires an approved permission review.');
        }
        const reviewed = new Set(permissionReview.reviewedPermissions);
        if (permissions.some((permission) => !reviewed.has(permission))) {
            throw new TypeError('Plugin permission review must cover every requested permission.');
        }
    }
    const manifest = {
        ...
        contractVersion: CAPABILITY_CONTRACT_VERSION,
        channel,
        packageVersion: cleanText(input.packageVersion, 80) || CAPABILITY_CONTRACT_VERSION,
        capability: {
            id: capabilityId,
            kind: projectCapabilityKindForPackage(sourceKind, channel),
            sourceKind,
            name: cleanText(input.capability?.name, 160) ||
                `Untitled ${sourceKind}`,
...
export function assertCapabilityPackageManifest(value) {
    ...
    if (value.schemaVersion !== CAPABILITY_PACKAGE_SCHEMA_VERSION) {
        throw new TypeError(`Unsupported capability package schema: ${String(value.schemaVersion || 'missing')}.`);
    }
    if (value.contractVersion !== CAPABILITY_CONTRACT_VERSION) {
        throw new TypeError(`Unsupported capability contract version: ${String(value.contractVersion || 'missing')}.`);
    }
    ...
    assertOnlyKeys(value.capability, ['id', 'kind', 'sourceKind', 'name', 'description'], 'Capability package capability');
    ...
    if (value.capability.kind !== projected) {
        throw new TypeError('Capability package kind does not match its channel projection.');
    }
    if (value.channel === 'publish' &&
    ...
    if (!isUniqueStringArray(value.permissions) ||
        !isUniqueStringArray(value.requiredSecrets) ||
        value.requiredSecrets.some((name) => !/^[A-Z][A-Z0-9_]{1,127}$/.test(name))) {
        throw new TypeError('Capability package permissions and requiredSecrets must be arrays.');
    }
    if (!isRecord(value.compatibility)) {
        throw new TypeError('Capability package compatibility is invalid.');
    }
    assertOnlyKeys(value.compatibility, ['hosts', 'platforms'], 'Capability package compatibility');
    if (!isUniqueStringArray(value.compatibility.hosts) ||
    ...
    if (value.permissionReview !== undefined) {
        ...
        assertOnlyKeys(value.permissionReview, ['status', 'reviewedPermissions', 'reviewer'], 'Capability package permissionReview');
        if (value.permissionReview.status !== 'approved' ||
    ...
    if (sourceKind === 'plugin' && value.channel === 'publish') {
        ...
        if (value.permissions.some((permission) => typeof permission !== 'string' || !reviewed.has(permission))) {
            throw new TypeError('Plugin permission review must cover every requested permission.');
        }
    }
    return value;
}
export function assertPublicCapabilityValue(value, path = []) {
    if (typeof value === 'string') {
        if (PRIVATE_PATH_PATTERN.test(value) ||
            ...
            throw new TypeError(`Public capability manifest contains private content at ${formatPath(path)}.`);
        }
        return;
    }
    if (Array.isArray(value)) {
        value.forEach((entry, index) => assertPublicCapabilityValue(entry, [...path, String(index)]));
        return;
    }
    if (!isRecord(value))
        return;
    for (const [key, entry] of Object.entries(value)) {
        if (FORBIDDEN_PUBLIC_KEYS.has(key)) {
            throw new TypeError(`Public capability manifest contains forbidden field ${formatPath([...path, key])}.`);
        }
...
export function hashCanonicalCapability(value) {
    return createHash('sha256')
        .update(canonicalCapabilityJson(value))
        .digest('hex');
}
function summarizeCapabilityItems(items) {
    const byKind = Object.fromEntries(CAPABILITY_KINDS.map((kind) => [
        kind,
    ...
    return {
        ...
        mcpCount: byKind.mcp,
        importEligibleCount: items.filter((item) => item.policy.import.eligibility === 'eligible').length,
        publishReviewCount: items.filter((item) => item.policy.publish.eligibility === 'review-required').length,
        publishBlockedCount: items.filter((item) => item.policy.publish.eligibility === 'blocked').length,
    };
}
function normalizePackageFiles(value) {
    if (value === undefined)
        return [];
    if (!Array.isArray(value)) {
        throw new TypeError('Capability package files must be an array.');
    ...
    return value
        ...
        assertOnlyKeys(entry, ['path', 'size', 'sha256'], 'Capability package file');
        const filePath = String(entry.path || '').replaceAll('\\', '/');
        const size = Number(entry.size);
        const sha256 = cleanText(entry.sha256, 128).toLowerCase();
        if (!RELATIVE_PACKAGE_PATH_PATTERN.test(filePath) ||
            !Number.isSafeInteger(size) ||
            size < 0 ||
            !SHA256_PATTERN.test(sha256)) {
            throw new TypeError(`Capability package file "${filePath || 'missing'}" is invalid.`);
        }
        return { path: filePath, size, sha256 };
    })
        .sort((first, second) => first.path.localeCompare(second.path));
}
function normalizePluginPermissionReview(value) {
...
function inferSourceFormat(kind, source, localPath) {
    if (kind === 'skill')
...
function compareCapabilityItems(first, second) {
