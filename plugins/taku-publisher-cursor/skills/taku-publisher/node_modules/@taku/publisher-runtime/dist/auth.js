...
export async function resolveAuth(options = {}) {
    ...
    if (envToken)
        return resolvedEnv(envToken, `env:${tokenEnv}`);
    if (tokenEnv === 'TAKU_BEARER_TOKEN') {
        const fallback = String(env.TAKU_PUBLISH_TOKEN ?? '').trim();
        if (fallback)
            return resolvedEnv(fallback, 'env:TAKU_PUBLISH_TOKEN');
    }
    ...
    if (publisherSession && !isExpired(publisherSession)) {
        return {
            ...
            scopes: Array.isArray(publisherSession.scopes)
                ? publisherSession.scopes.filter((scope) => typeof scope === 'string' && Boolean(scope.trim()))
                : [],
    ...
    if (!session)
        return { token: '', source: 'missing', iconToken: '', flowchartToken: '', scopes: [], sessionPath: desktopPath, refreshed: false };
    ...
    if (refreshed) {
        ...
        if (token)
            return { token, source: 'session', iconToken: token, flowchartToken: token, scopes: [], sessionPath: desktopPath, refreshed: true };
    }
...
export async function authStatus(options = {}) {
    ...
    if (!session)
        session = readSession(desktopPath);
...
export function publisherAccountHint(options = {}) {
    const env = options.env ?? process.env;
    const tokenEnv = options.tokenEnv ?? 'TAKU_BEARER_TOKEN';
    if (options.authSource && !['session', 'publisher_session'].includes(options.authSource))
        return null;
    if (String(env[tokenEnv] ?? '').trim())
        return null;
    if (tokenEnv === 'TAKU_BEARER_TOKEN' && String(env.TAKU_PUBLISH_TOKEN ?? '').trim())
        return null;
...
export function publisherSessionPath(env = process.env) {
...
export async function savePublisherSession(payload, env = process.env) {
    const target = publisherSessionPath(env);
    await atomicWriteJson(target, payload, 0o600);
    return target;
}
export async function clearPublisherSession(env = process.env) {
    try {
...
export function readSession(target = sessionPath()) {
    try {
...
export async function refreshSession(session, options = {}) {
    ...
    try {
        const response = await (options.transport ?? defaultRefreshTransport)(`${TAKU_ACCOUNT_BASE_URL}/auth/v1/token?grant_type=refresh_token`, headers, body, options.timeoutMs ?? 15_000);
...
function resolvedEnv(token, source) {
    return {
        token,
        source,
...
async function defaultRefreshTransport(url, headers, body, timeoutMs) {
...
function validIconToken(session) {
...
function validFlowchartToken(session) {
