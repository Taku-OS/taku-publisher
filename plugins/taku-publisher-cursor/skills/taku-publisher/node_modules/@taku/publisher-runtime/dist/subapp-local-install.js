...
import { atomicWriteJson, isRecord, PublisherError, sha256File, sortJson, } from './util.js';
...
function asPositiveInteger(value, label, maximum) {
...
async function validatePackage(packageRootInput) {
    const requested = path.resolve(String(packageRootInput ?? '').trim());
    const rootStat = await fsp.lstat(requested).catch(() => null);
    if (!rootStat?.isDirectory() || rootStat.isSymbolicLink()) {
        throw new PublisherError('SubApp package root must be an existing local directory.', 'subapp_local_install_package_invalid');
    }
    ...
    if (names.length !== expectedNames.length || names.some((name, index) => name !== expectedNames[index])) {
        throw new PublisherError('SubApp package root contains unexpected files.', 'subapp_local_install_package_invalid');
    }
    ...
    let value;
    try {
        value = JSON.parse(await fsp.readFile(manifestPath, 'utf8'));
    }
    catch {
    ...
    if (sourceValue.fileName !== SOURCE_FILE ||
        ...
        throw new PublisherError('SubApp package does not match the local Taku installation contract.', 'subapp_local_install_contract_invalid');
    }
...
function confirmationToken(validated) {
...
function resolveHandoffRoot(options) {
    if (options.handoffRoot)
        return path.resolve(options.handoffRoot);
    return path.join(os.homedir(), '.taku', 'publisher', 'subapp-handoffs');
}
export async function planLocalSubAppInstall(request) {
...
async function openTakuDeepLink(url, takuAppPath) {
    let packagedAppPath = '';
    if (takuAppPath) {
        if (process.platform !== 'darwin') {
            throw new PublisherError('An explicit packaged Taku app path is currently supported only on macOS.', 'taku_desktop_path_invalid');
        }
        ...
        if (!stat?.isDirectory() || stat.isSymbolicLink() || !requested.toLowerCase().endsWith('.app')) {
            throw new PublisherError('The explicit Taku Desktop path must be an existing packaged .app directory.', 'taku_desktop_path_invalid');
        }
        ...
        if (!infoPlist?.isFile() || !macOsDir?.isDirectory()) {
            throw new PublisherError('The explicit Taku Desktop path is not a valid packaged macOS app.', 'taku_desktop_path_invalid');
        }
...
async function waitForResult(resultPath, handoffId, options) {
...
export async function requestLocalSubAppInstall(request, options = {}) {
    const validated = await validatePackage(request.packageRoot);
    if (request.confirmationToken !== confirmationToken(validated)) {
        throw new PublisherError('The local SubApp install confirmation is missing, stale, or belongs to different package bytes.', 'subapp_local_install_confirmation_mismatch');
    }
    ...
    const runtimeManifest = validated.manifest.runtimeManifest;
    ...
    if (!opened) {
        await fsp.rm(handoffPath, { force: true }).catch(() => undefined);
        throw new PublisherError('Taku could not be opened. Install or update the Taku desktop app and try again.', 'taku_desktop_unavailable');
    }
