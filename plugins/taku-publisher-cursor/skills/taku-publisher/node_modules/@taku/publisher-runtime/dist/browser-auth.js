...
export async function loginWithBrowser(options) {
    ...
    if (!address || typeof address === 'string')
        throw new PublisherError('Unable to start local authorization callback.', 'auth_callback_failed');
    ...
    process.stderr.write('Waiting for browser confirmation. Complete Taku sign-in or authorization in the opened page; this command will continue automatically.\n');
    const browserOpened = options.openBrowser !== false
        && await (options.browserOpen ?? openExternal)(loginUrl);
    let manualBrowserFallbackTimer;
    const printManualBrowserFallback = () => {
        process.stderr.write(`If the Taku authorization page is not visible, open this URL manually:\n${loginUrl}\n`);
    };
    if (!browserOpened) {
        ...
        manualBrowserFallbackTimer = setTimeout(printManualBrowserFallback, Math.max(0, options.manualBrowserFallbackDelayMs ?? DEFAULT_MANUAL_BROWSER_FALLBACK_DELAY_MS));
    }
    let received;
    let timeoutId;
    try {
        received = await Promise.race([
            ...
            new Promise((_, reject) => {
                timeoutId = setTimeout(() => reject(new PublisherError('Browser confirmation timed out. Run the original command again to retry authorization.', 'auth_timeout', { browser_opened: browserOpened })), Math.max(1_000, options.timeoutMs ?? DEFAULT_LOGIN_TIMEOUT_MS));
            }),
        ]);
    }
    finally {
        if (manualBrowserFallbackTimer)
            clearTimeout(manualBrowserFallbackTimer);
        if (timeoutId)
    ...
    if (!accessToken)
        throw new PublisherError('Taku Web did not return a publisher authorization.', 'invalid_auth_response');
    const now = Date.now();
...
export function buildLoginUrl(options) {
    ...
    if (options.accountMode)
        query.set('account_mode', options.accountMode);
...
async function redeemLocalCode(options) {
    try {
        ...
        if (!isRecord(payload))
            throw new PublisherError('Taku Web returned an invalid authorization response.', 'invalid_auth_response');
        if (!response.ok)
            throw new PublisherError(String(payload.error ?? `Authorization failed with HTTP ${response.status}`), 'auth_redeem_failed');
        return payload;
    }
    catch (error) {
