# Cursor and Agent Skills installer 0.3.23

This enables automatic Flowchart generation for new Publisher drafts by
default while preserving creator-provided Flowcharts and an explicit operator
opt-out. It also includes the Cursor, Agent Skills, OpenCode, Stax Challenge,
and SubApp support from 0.3.22. The production plugin is still named
...
```sh
npx --yes --package ./taku-publisher-0.3.23.tgz taku-publisher install --host cursor
npx --yes --package ./taku-publisher-0.3.23.tgz taku-publisher install --host agent-skills
```
...
The local npm tarball is `dist/releases/taku-publisher-0.3.23.tgz`:

```sh
npx --yes --package ./dist/releases/taku-publisher-0.3.23.tgz taku-publisher install --host cursor
npx --yes --package ./dist/releases/taku-publisher-0.3.23.tgz taku-publisher install --host agent-skills
```

This tarball command uses a local file, not a released npm package. A public
`npx --yes --package @taku/publisher@0.3.23 taku-publisher install --host cursor`
